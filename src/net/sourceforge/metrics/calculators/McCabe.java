 ...
 *
 *
 * $Id: McCabe.java,v 1.13 2004/04/30 20:21:17 sauerf Exp $
 */
	...
	public void calculate(AbstractMetricSource source)
		...
		if (source.getLevel() != METHOD)
			throw new InvalidSourceException(
					"McCabe only applicable to methods");
		...
		sourceCode = internalCalculate(source, sourceCode);
		McCabeVisitor mcb = new McCabeVisitor(sourceCode);
		astNode.accept(mcb);
		source.setValue(new Metric(getName(), mcb.cyclomatic));
	}

	private String internalCalculate(AbstractMetricSource source,
			String sourceCode) {
		return source.getCompilationUnit().getSource();
