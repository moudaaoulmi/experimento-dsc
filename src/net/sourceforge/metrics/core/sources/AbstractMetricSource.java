...
/**
 * Base class for all metric sources. Used by calculators to retrieve the AST,
 * IJavaElement, etc. and to store their results.
 * 
 * @author Frank Sauer
 */
public abstract class AbstractMetricSource implements Constants, Serializable {
	...
	public AbstractMetricSource[] getChildrenHaving(String per, String metric) {
		...
		for (Iterator i = childHandles.iterator(); i.hasNext();) {
			...
			if (elm != null) {
				AbstractMetricSource next = Dispatcher
						.getAbstractMetricSource(elm);
				if (next != null) {
		...
		return (AbstractMetricSource[]) result
				.toArray(new AbstractMetricSource[] {});
	}

	public String getName() {
		return (getJavaElement() == null) ? "" : getJavaElement()
				.getElementName();
	}
	...
	public void initializeNewInstance(AbstractMetricSource newSource,
			IJavaElement element, Map data) {
		newSource.childHandles.clear();
		newSource.setJavaElement(element);
	}

	/**
	 * Used by Calculators to store their result metrics. As a side effect, adds
	 * a marker of type net.sourceforge.metrics.outofrangemarker if enabled and
	 * the value is out of the range indicated by the metric's descriptor
	 * 
	 * @param value
	 */
	...
	private void checkRange(Metric value) {
		MetricDescriptor md = MetricsPlugin.getDefault().getMetricDescriptor(
				value.getName());
		if (!md.isValueInRange(value.doubleValue())) {
			internalCheckRange(value, md);
		}
	}

	private void internalCheckRange(Metric value, MetricDescriptor md) {
		IResource resource = getJavaElement().getUnderlyingResource();
		if (resource != null)
			configureOutOfRangeMarker(value, md, resource);
	}

	private void configureOutOfRangeMarker(Metric value, MetricDescriptor md,
			IResource resource) throws CoreException {
		IMarker marker = resource
				.createMarker("net.sourceforge.metrics.outofrangemarker");
		if ((marker != null) && (marker.exists())) {
			JavaCore.getJavaCore().configureJavaElementMarker(marker,
					getJavaElement());
			...
			if (getJavaElement() instanceof IMember) {
				...
				if (cu != null) {
					marker.setAttribute(IMarker.LINE_NUMBER, cu
							.lineNumber(offset));
				}
			}
		}
	...
	/**
	 * Answers whether the current thread has been interrupted. Used to
	 * prematurely abort a long calculation
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Get a list of all metrics defined in this node's children with the given
	 * name
	 * 
	 * @param name
	 * @return List
	...
	public List getMetricsFromChildren(String name) {
		...
		for (Iterator i = getChildren().iterator(); i.hasNext();) {
			...
			if (m != null)
				...
				System.err.println("metric " + name + " not found in "
						+ next.getJavaElement().getElementName());
			}
		}
		return metrics;
	}
	...
	public List getAveragesFromChildren(String name, String per) {
		...
		for (Iterator i = getChildren().iterator(); i.hasNext();) {
			...
			if (nextAvg != null) {
				...
				System.err.println("average " + name + "," + per
						+ " not found in "
						+ next.getJavaElement().getElementName());
			}
		}
	...
	/**
	 * Get the AST of this node. Used frequently by calculators. This base class
	 * returns null. Implemented by CompilationUnitMetrics
	 * 
	 * @return IJavaElement
	...
	/**
	 * get the original compilation unit for the given IJavaElement. If the
	 * compilation unit turns out to be a WorkingCopy this method returns its
	 * original source compilation unit
	 * 
	 * @param input
	 * @return ICompilationUnit
	 */
	...
	/**
	 * invokes calculate() on all calculators. TODO fine-grained Cache lookup so
	 * we can add new metrics
	 */
