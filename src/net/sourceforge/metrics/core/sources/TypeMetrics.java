...
public class TypeMetrics extends AbstractMetricSource {
	...
	protected void initializeChildren() {
		...
		for (int i = 0; i < methods.length; i++) {
			...
			MethodMetrics mm = (MethodMetrics) Dispatcher
					.calculateAbstractMetricSource(method, this, data);
			addChild(mm);
		}
	}
	...
	private IMethod findMethod(MethodDeclaration m) {
		...
		for (Iterator i = parms.iterator(); i.hasNext(); index++) {
			...
			// FIX F.S. 2/23/03 - without, invalid handles are produced by found
			// method
	...
	public ITypeHierarchy getHierarchy() {
		if (hierarchy == null) {
			...
			internalGetHierarchy(iType);
		}
		return hierarchy;
	}

	private void internalGetHierarchy(IType iType) {
