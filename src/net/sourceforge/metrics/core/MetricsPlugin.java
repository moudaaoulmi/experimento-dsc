 ...
 * @author Frank Sauer
 */
public class MetricsPlugin extends AbstractUIPlugin implements
		IPropertyChangeListener, Constants {
	...
	private static URL makeImageURL(String name) {
		...
		URL url = null;
		url = new URL(MetricsPlugin.getDefault().getDescriptor()
				.getInstallURL(), path);
		return url;
	}
	...
	public MetricsPlugin(IPluginDescriptor descriptor) {
		super(descriptor);
		plugin = this;
		internalMetricsPlugin();
		installExtensions();
	}

	private void internalMetricsPlugin() {
		resourceBundle = ResourceBundle
				.getBundle("net.sourceforge.metrics.core.MetricsPluginResources");
	}
	...
	private String[] parsePrefString(boolean description) {
		String stringList = getPreferenceStore().getString(
	...
	public static long lastTimePreferencesChanged() {
		return getDefault().getPreferenceStore().getLong(
				"METRICS.lastPrefChange");
	}
	...
	public static void recordTimeAndClearCache() {
		...
		getDefault().getPreferenceStore().putValue("METRICS.lastPrefChange",
	...
	public IPreferenceStore getPreferenceStore() {
		if (!listenerAdded) {
			super.getPreferenceStore().setDefault("METRICS.decimals",
					FRACTION_DIGITS);
			super
					.getPreferenceStore()
					.setDefault("METRICS.xmlformat",
							"net.sourceforge.metrics.internal.xml.MetricsFirstExporter");
			super.getPreferenceStore().setDefault("METRICS.enablewarnings",
					false);
			super.getPreferenceStore().setDefault("METRICS.defaultColor",
					"0,0,0");
			super.getPreferenceStore().setDefault("METRICS.linkedColor",
					"0,0,255");
			super.getPreferenceStore().setDefault("METRICS.outOfRangeColor",
					"255,0,0");
			super.getPreferenceStore().setDefault("METRICS.depGR_background",
					"1,17,68");
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		Object[] l = null;// listeners.getListeners();
	...
	private void installMetrics() {
		...
		if (p != null) {
			...
			for (int i = 0; i < x.length; i++) {
				IConfigurationElement[] elements = x[i]
	...
	public ExportDescriptor[] getExporters() {
		return (ExportDescriptor[]) exporters.values().toArray(
	...
	/**
	 * Create MaxMax, AvgAvg and Sum calculators at each level above the given
	 * level
	 * 
	 * @param level
	 * @param p
	 */
	private void propagatePropagator(String level, Propagator p,
			MetricDescriptor m) {
	 ...
	 */
	private CalculatorDescriptor createDescriptorFrom(
	...
	/**
	 * returns a list of metric ids that depend on the given metric being
	 * available
	 * 
	 * @param descriptor
	...
	public String[] getDependentMetrics(MetricDescriptor descriptor) {
		...
		if (dependents != null)
			return (String[]) dependents.toArray(new String[] {});
		return null;
	}
