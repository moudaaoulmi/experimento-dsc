...
public class ProgressQueue extends LinkedList {
	...
	/**
	 * Add listener l to be notified of metrics progress. If there are any
	 * calculations already ongoing, l will be caught up by the immediate
	 * receipt on the current thread of a queued event with the current number
	 * of outstanding calculations.
	 * 
	 * @param l
	 */
	public void addMetricsProgressListener(IMetricsProgressListener l) {
		if ((l != null) && (!listeners.contains(l))) {
			...
			synchronized (items) {
				if (items.size() > 0) {
					Log
							.logMessage("Catching up new metrics progress listener with "
									+ items.size() + " items.");
	...
	/**
	 * Queue the command unless queue is paused and the command is not a command
	 * that triggers resume (QueueCommand)
	 * 
	 * @param command
	 */
	...
	/**
	 * Get the first queued command. Pauses the queue if this command is a
	 * pausing command (AbortCommand). Throws InterruptedException if the Queue
	 * is blocked on its Semaphore and the thread was interrupted.
	 * 
	...
	private class NotifierThread extends Thread {
		...
		private void internal(Command next) {
			next.execute();
		}
	...
	private class PendingCommand extends Command {
		...
		void execute() {
			for (Iterator i = listeners.iterator(); i.hasNext();) {
				IMetricsProgressListener next = (IMetricsProgressListener) i
						.next();
