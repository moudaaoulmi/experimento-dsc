...
/**
 * Action for the checked menu item in the IJavaproject popup. Checks/Unchecks
 * based on the presence of the metrics nature and selection adds/removes the
 * nature.
 * 
 * @author Frank Sauer
 */
public class EnableMetrics implements IObjectActionDelegate, Constants {
	...
	public void run(IAction action) {
		...
		if (project != null) {
			IRunnableWithProgress op = new IRunnableWithProgress() {
				...
				// tratamento impossibilitado por se tratar de uma Interface
				public void run(IProgressMonitor monitor)
					...
					try {
						if (!project.hasNature(pluginId + ".nature")) {
							...
							monitor.worked(1);
							error = "disable";
							MetricsNature.removeNatureFromProject(project,
									monitor);
			...
			internalRun(shell, op);
		}
	}

	private void internalRun(Shell shell, IRunnableWithProgress op) {
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((!selection.isEmpty())
			...
			Openable op = (Openable) ((IStructuredSelection) selection)
					.getFirstElement();
			if (op != null) {
