...
public class MetricsFirstExporter implements IExporter, Constants {
	...
	private void printRoot(AbstractMetricSource root, XMLPrintStream pOut,
		...
		pOut
				.println("\" xmlns=\"http://metrics.sourceforge.net/2003/Metrics-First-Flat\">");
		...
		for (int i = 0; i < names.length; i++) {
			...
			pOut.println(">");
			if (!printValues(names[i], root, pOut)) {
				Metric val = root.getValue(names[i]);
				if (val != null) {
					pOut.indent(2);
					pOut.print("<Value value=\"");
					pOut.print(val.doubleValue());
					pOut.println("\"/>");
				}
			}
			pOut.indent(1);
	...
	/**
	 * If root is a source folder or a project, include any cyclic package
	 ...
	 * @param monitor
	 */
	private void maybePrintCycles(AbstractMetricSource root,
			XMLPrintStream pOut, IProgressMonitor monitor) {
	...
	private void exportCycle(StrongComponent component, XMLPrintStream pOut) {
		...
		if (nodes > 1) {
			...
			String name = component.getVertex(0).getAttributes().toString()
					+ " et al";
			pOut.indent(1);
			...
			for (int i = 0; i < nodes; i++) {
				String packName = component.getVertex(i).getAttributes()
						.toString();
				pOut.indent(2);
				pOut.print("<Package>");
				pOut.print(packName);
	...
	private Class[] filters = new Class[] { PackageFragmentMetrics.class,
			TypeMetrics.class, MethodMetrics.class };

	private boolean printValues(String id, AbstractMetricSource element,
			XMLPrintStream pOut) {
		...
		for (int i = 0; i < pers.length; i++) {
			...
			if ((avg != null) || (max != null)) {
				...
				if (total != null) {
					pOut.print(" total = \"");
					pOut.print(total.doubleValue());
					pOut.print("\"");
				}
				if (avg != null) {
					pOut.print(" avg = \"");
					pOut.print(avg.doubleValue());
					pOut.print("\"");
					pOut.print(" stddev = \"");
					pOut.print(avg.getStandardDeviation());
					pOut.print("\"");
				}
				if (max != null) {
					pOut.print(" max = \"");
	 ...
	 * @param pOut
	 */
	private void printValues(List values, final String id, XMLPrintStream pOut,
			MetricDescriptor md) {
		// sort values first
	...
	/**
	 * keys are strong, values are soft, strangely enough WeakHashMap does the
	 * opposite??? What good is that?
	 * 
	 * @author Frank Sauer
	 */
