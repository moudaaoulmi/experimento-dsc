...
import ish.ecletex.wordnet.jwnl.dictionary.file.DictionaryCatalog;
...
import ish.ecletex.wordnet.jwnl.dictionary.file.ObjectDictionaryFile;
import ish.ecletex.wordnet.jwnl.util.factory.Param;

import java.util.*;

/**
 * A <code>Dictionary</code> backed by <code>Map</code>s. Warning: this has huge
 * memory requirements. Make sure to start the interpreter with a large enough
 * free memory pool to accomodate this.
 */
public class MapBackedDictionary extends Dictionary {
	...
	/**
	 * File type install parameter. The value should be * the name of the
	 * appropriate subclass of <code>DictionaryFileType</code>.
	 */
	...
	public static void install(String searchDir, Class dictionaryFileType)
			throws JWNLException {
		install(searchDir, dictionaryFileType, null);
	}

	public static void install(String searchDir, Class dictionaryFileType,
			MorphologicalProcessor morph) throws JWNLException {
		checkFileType(dictionaryFileType);
		DictionaryCatalogSet files = new DictionaryCatalogSet(searchDir,
				dictionaryFileType);
	...
	/**
	 * Install a <code>MapBackedDictionary</code> from a map of parameters. The
	 * parameters are chosen from the static variables above.
	 */
	public void install(Map params) throws JWNLException {
		...
		dictionaryFileType = internalInstall(param, dictionaryFileType);
		checkFileType(dictionaryFileType);
	...
	private Class internalInstall(Param param, Class dictionaryFileType)
			throws JWNLException {
	...
	/**
	 * Create a <code>MapBackedDictionary</code> with the specified set of
	 * files.
	 */
	private MapBackedDictionary(DictionaryCatalogSet files,
			MorphologicalProcessor morph) throws JWNLException {
		super(morph);
		if (!files.isOpen()) {
			internalMapBackedDictionary(files);
		}
		// Load all the hashtables into memory

		for (Iterator typeItr = DictionaryFileType.getAllDictionaryFileTypes()
				.iterator(); typeItr.hasNext();) {
	...
	private void internalMapBackedDictionary(DictionaryCatalogSet files)
			throws JWNLException {
		files.open();
	}

	public IndexWord getIndexWord(POS pos, String lemma) {
		return (IndexWord) getTable(pos, DictionaryFileType.INDEX).get(
				prepareQueryString(lemma));
	}
	...
	public IndexWord getRandomIndexWord(POS pos) throws JWNLException {
		int index = _rand.nextInt(getTable(pos, DictionaryFileType.INDEX)
				.size());
	...
	public Synset getSynsetAt(POS pos, long offset) {
		return (Synset) getTable(pos, DictionaryFileType.DATA).get(
				new Long(offset));
	}

	public Exc getException(POS pos, String derivation) {
		return (Exc) getTable(pos, DictionaryFileType.EXCEPTION).get(
				prepareQueryString(derivation));
	}
	...
	/**
	 * Use <var>table</var> for lookups to the file represented by
	 * <var>pos</var> and <var>fileType</var>.
	 */
	...
	private static final class IndexWordIterator implements Iterator {
		...
		public IndexWordIterator(Iterator itr, String searchString,
				String startWord) {
			_itr = itr;
