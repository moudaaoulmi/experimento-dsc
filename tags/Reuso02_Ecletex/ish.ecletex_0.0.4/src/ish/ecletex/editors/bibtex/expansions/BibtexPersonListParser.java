...
import ish.ecletex.editors.bibtex.dom.*;
...
final class BibtexPersonListParser {
	...
	public static BibtexPersonList parse(BibtexString personList)
		...
		for (int i = 0; i < tokens.length; i++) {
			if (tokens[i].toLowerCase().equals(AND) && begin < i) {
				result.add(makePerson(tokens, begin, i, content, personList
						.getOwnerFile()));
				begin = i + 1;
			}
		}
		if (begin < tokens.length)
			result.add(makePerson(tokens, begin, tokens.length, content,
					personList.getOwnerFile()));
		return result;
	}

	private static BibtexPerson makePerson(String[] tokens, int begin, int end,
		...
		if (tokens[begin].equals("others")) {
			...
			if (numberOfCommas == 0) {
				...
				if (numberOfCommas == 1) {
					...
					if (!tokens[end - 1].equals("Jr"))
						throw new PersonListParserException(
								"Expected 'Jr' instead of '" + tokens[end - 1]
										+ "' in " + fullEntry);
					lineage = "Jr";
					end -= 2;
				}
				...
				}
				{
					...
					if (buffer.toString().trim().equals("")) {
						throw new PersonListParserException(
								"Last name empty in '" + fullEntry + "'");
					} else
						last = buffer.toString().trim();
					return factory.makePerson(first, preLast, last, lineage,
							false);
				}
			} else {
				throw new PersonListParserException("Too many commas in "
						+ fullEntry + ".");
			}
		}
	}
			...
			case ',':
				if (numberOfOpenBraces == 0) {
					if (tokenBegin <= currentPos - 1) {
						String potentialToken = stringContent.substring(
								tokenBegin, currentPos).trim();
