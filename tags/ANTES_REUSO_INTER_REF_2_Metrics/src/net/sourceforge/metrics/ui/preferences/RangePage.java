...
/**
 * preference page allowing the user to set safe ranges for the metrics and the
 * corresponding tip to solve out-of-range problems.
 * 
 * @author Frank Sauer
 */
public class RangePage extends PreferencePage implements
		IWorkbenchPreferencePage {

	// private static class MetricDescriptorLabelProvider extends LabelProvider
	...
	private static class RangeCellModifier implements ICellModifier {
		...
		public void modify(Object element, String property, Object value) {
			...
			String values[] = new String[] { item.getText(0), item.getText(1),
					item.getText(2), item.getText(3), item.getText(4) };
			if (property.equals("MIN")) {
				String minStr = (String) value;
				internal(values, minStr, 2);
			} else if (property.equals("MAX")) {
				String maxStr = (String) value;
				internal(values, maxStr, 3);
			} else if (property.equals("HINT")) {
		...
		private void internal(String[] values, String str, int i) {
	...
	private String[] properties = new String[] { "NAME", "LEVEL", "MIN", "MAX",
			"HINT" };
	...
	protected Control createContents(Composite ancestor) {
		...
		tv.setCellEditors(new CellEditor[] { null, null,
	...
	private void populateTable() {
		...
		// MetricDescriptor md =
		// MetricsPlugin.getDefault().getMetricDescriptor(ids[i]);
		// tv.add(md);
		// }
	}
		...
		if (MessageDialog
			...
			if (items != null) {
				for (int i = 0; i < items.length; i++) {
					// MetricDescriptor md =
					// (MetricDescriptor)items[i].getData();
					// if (md != null) {
					// md.resetToDefaults();
					// }
				}
