...
public class HardLineWrap {
	...
	public void doWrapB(IDocument d, DocumentCommand c, int MAX_LENGTH) {
		...
		String line = d.get(commandRegion.getOffset(), commandRegion
				.getLength());

		// Special case if there are white spaces at the end of the line
		if (trimEnd(line).length() + c.text.length() <= MAX_LENGTH)
			return;
		...
		// String indent = tools.getIndentation(d, c); // TODO check if inside
		// comment
