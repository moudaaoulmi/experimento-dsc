...
public class TexlipseAnnotationUpdater implements ISelectionChangedListener {
		...
		TexlipsePlugin.getDefault().getPreferenceStore()
				.addPropertyChangeListener(new IPropertyChangeListener() {
					...
					public void propertyChange(PropertyChangeEvent event) {
						...
						if (TexlipseProperties.TEX_EDITOR_ANNOTATATIONS
								.equals(property)) {
							boolean enabled = TexlipsePlugin
									.getDefault()
									.getPreferenceStore()
									.getBoolean(
											TexlipseProperties.TEX_EDITOR_ANNOTATATIONS);
	...
	private void update(ISourceViewer viewer) {
		...
		if (selection instanceof ITextSelection) {
			internalUpdate(document, model, selection);
		}
	}

	private void internalUpdate(final IDocument document,
			final IAnnotationModel model, ISelection selection) {
		...
		final String line = document.get(lineOff, document
				.getLineLength(lineNr));
		IRegion r = LatexParserUtils.getCommand(line, offset - lineOff);
		if (r == null)
			throw new SoftException(null);
		// return; para possibilitar o reuso
		final String command = line.substring(r.getOffset(),
				r.getOffset() + r.getLength()).trim();
		if ("\\begin".equals(command) || "\\end".equals(command)) {
			// TODO Its maybe better/faster to use the AST here
			IRegion r2 = LatexParserUtils.getCommandArgument(line, r
					.getOffset());
			if (r2 == null)
				throw new SoftException(null);
			// return; para possibilitar o reuso

			final IRegion startRegion = new Region(lineOff + r.getOffset(), r2
					.getOffset()
					+ r2.getLength() - r.getOffset() + 1);

			final String refName = line.substring(r2.getOffset(), r2
					.getOffset()
					+ r2.getLength());
			// Create a job to update the annotations in the background
			fUpdateJob = createMatchEnvironmentJob(document, model, offset,
					command, startRegion, refName);
	 ...
	 * @return The job
	 */
	private Job createMatchReferenceJob(final IDocument document,
			final IAnnotationModel model, final String refName) {
		return new Job("Update Annotations") {
			public IStatus run(IProgressMonitor monitor) {
				String text = document.get();
				String refNameRegExp = refName.replaceAll("\\*", "\\\\*");
				final String simpleRefRegExp = "\\\\([a-zA-Z]*ref|label)\\s*\\{"
						+ refNameRegExp + "\\}";
				Matcher m = (Pattern.compile(simpleRefRegExp)).matcher(text);
				while (m.find()) {
					if (monitor.isCanceled())
						return Status.CANCEL_STATUS;
					IRegion match = LatexParserUtils
							.getCommand(text, m.start());
	 ...
	 * @return The Job
	 */
	private Job createMatchEnvironmentJob(final IDocument document,
			final IAnnotationModel model, final int offset,
			final String command, final IRegion startRegion,
		...
		return new Job("Update Annotations") {
			public IStatus run(IProgressMonitor monitor) {
				...
				if ("\\begin".equals(command))
					forward = true;
				if (forward) {
					IRegion endRegion = LatexParserUtils
							.findMatchingEndEnvironment(text, envName,
					...
					IRegion endRegion = LatexParserUtils
							.findMatchingBeginEnvironment(text, envName,
									startRegion.getOffset());
					if (endRegion != null) {
						createNewAnnotation(endRegion, "Environment", model);
						createNewAnnotation(startRegion, "Environment", model);
	...
	private boolean testSelection(ISelection selection, IAnnotationModel model) {
		if (selection instanceof ITextSelection) {
			...
			for (Iterator<Annotation> iter = fOldAnnotations.iterator(); iter
					.hasNext();) {
	 ...
	 *            The AnnotationModel
	 */
	private void createNewAnnotation(IRegion r, String annString,
