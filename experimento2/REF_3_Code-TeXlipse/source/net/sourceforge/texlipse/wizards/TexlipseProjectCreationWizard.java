/*
 * $Id: TexlipseProjectCreationWizard.java,v 1.1 2005/03/16 12:22:32 kimmok Exp $
 *
 ...
 * @author kpkarlss
 */
public class TexlipseProjectCreationWizard extends Wizard implements
	...
	public boolean performFinish() {
		...
		TexlipseProjectCreationOperation runnable = new TexlipseProjectCreationOperation(
				attributes);
		IRunnableWithProgress op = new WorkspaceModifyDelegatingOperation(
				runnable);

		boolean result = true;
		result = internalPerformFinish(op, result);
	...
	private void handleTargetException(Throwable target) {
		...
		if (target instanceof CoreException) {
			...
			ErrorDialog.openError(getShell(), TexlipsePlugin
					.getResourceString("projectWizardErrorTitle"),
					TexlipsePlugin
							.getResourceString("projectWizardErrorMessage"),
					status);

		} else {

			MessageDialog.openError(getShell(), TexlipsePlugin
					.getResourceString("projectWizardErrorTitle"), target
					.getMessage());
		}
	}
