/*
 * $Id: BibStringCompleter.java,v 1.3 2007/10/05 18:20:08 borisvl Exp $
 *
	...
	public void documentChanged(DocumentEvent event) {
		if (TexlipsePlugin.getDefault().getPreferenceStore().getBoolean(
			...
			if ("{".equals(event.getText())) {
				...
				internalDocumentChanged(textSelection);
			} else if ("\"".equals(event.getText()) && !recentlyAdded) {
				ITextSelection textSelection = (ITextSelection) this.editor
						.getSelectionProvider().getSelection();
				internalDocumentChanged2(textSelection);
				recentlyAdded = false;
			}
		}
	}

	private void internalDocumentChanged2(ITextSelection textSelection) {
		recentlyAdded = true;
		document.replace(textSelection.getOffset() + 1, 0, "\"");
	}

	private void internalDocumentChanged(ITextSelection textSelection) {
