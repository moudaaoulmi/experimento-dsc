/*
 * $Id: BibOutlinePage.java,v 1.2 2006/04/15 19:23:27 oskarojala Exp $
 *
...
public class BibOutlinePage extends ContentOutlinePage {
	...
	public void createControl(Composite parent) {
		...
		viewer.setContentProvider(new BibContentProvider(editor
	...
	private void internalSelectionChanged(ISelection selection) {
		ReferenceEntry be = (ReferenceEntry) ((IStructuredSelection) selection)
				.getFirstElement();
	...
	private void createActions() {
		...
		Action byAuthor = new Action("Sort by Author", IAction.AS_RADIO_BUTTON) {
			public void run() {
				if (isChecked()) {
					BibOutlineContainer newContent = ((BibContentProvider) getTreeViewer()
							.getContentProvider())
		...
		byAuthor.setImageDescriptor(TexlipsePlugin
				.getImageDescriptor("sortauthor"));
		this.outlineActions.put(ACTION_BYAUTHOR, byAuthor);

		Action byJournal = new Action("Sort by Journal",
		...
		byJournal.setImageDescriptor(TexlipsePlugin
				.getImageDescriptor("sortjournal"));
		...
		byIndex.setImageDescriptor(TexlipsePlugin
				.getImageDescriptor("sortindex"));
		...
		byYear
				.setImageDescriptor(TexlipsePlugin
						.getImageDescriptor("sortyear"));
		...
		openBibRef
				.setToolTipText("Open the current reference in an external viewer");
