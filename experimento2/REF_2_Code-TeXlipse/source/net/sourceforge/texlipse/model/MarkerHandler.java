...
public class MarkerHandler {
	 ...
	 */
	public void createErrorMarkers(ITextEditor editor,
	 ...
	 *            <code>ParseErrorMessage</code>
	 */
	public void createTaskMarkers(ITextEditor editor,
			List<ParseErrorMessage> tasks) {
	 ...
	 */
	private void createMarkers(ITextEditor editor,
			List<ParseErrorMessage> markers, final String markerType) {
		...
		for (ParseErrorMessage msg : markers) {
			internalCreateMarkers(markerType, resource, document, msg);
		}
	}

	private void internalCreateMarkers(final String markerType,
			IResource resource, IDocument document, ParseErrorMessage msg) {
		int beginOffset = document.getLineOffset(msg.getLine() - 1)
				+ msg.getPos();
		...
		map.put(IMarker.CHAR_END, Integer
				.valueOf(beginOffset + msg.getLength()));
	 ...
	 */
	public void createReferencingErrorMarkers(ITextEditor editor,
			List<DocumentReference> errors) {
		...
		for (DocumentReference msg : errors) {
			internalCreateReferencingErrorMarkers(resource, document, msg);
		}
	}

	private void internalCreateReferencingErrorMarkers(IResource resource,
			IDocument document, DocumentReference msg) {
		int beginOffset = document.getLineOffset(msg.getLine() - 1)
				+ msg.getPos();
	...
	public void addFatalError(ITextEditor editor, String error) {
		...
		internalAddFatalError(error, resource);
	}

	private void internalAddFatalError(String error, IResource resource) {
		Map<String, ? super Object> map = new HashMap<String, Object>();
	...
	public void clearErrorMarkers(ITextEditor editor) {
		...
		internalClearErrorMarkers(resource);
	}

	private void internalClearErrorMarkers(IResource resource) {
	...
	public void clearTaskMarkers(ITextEditor editor) {
		...
		if (resource == null)
			return;
		internalClearTaskMarkers(resource);
	}

	private void internalClearTaskMarkers(IResource resource) {
	 ...
	 */
	public void createErrorMarker(IResource resource, String message,
			int lineNumber) {
		Map<String, ? super Object> map = new HashMap<String, Object>();
