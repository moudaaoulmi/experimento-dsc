...
public class TeXSpellingReconcileStrategy extends SpellingReconcileStrategy {
	 ...
	 */
	private static class TeXSpellingProblemCollector implements
		...
		public TeXSpellingProblemCollector(IAnnotationModel annotationModel,
			...
			if (fAnnotationModel instanceof ISynchronizable)
				fLockObject = ((ISynchronizable) fAnnotationModel)
						.getLockObject();
			else
		...
		public void accept(SpellingProblem problem) {
			...
			internalAccept(start);
			fAddAnnotations.put(new SpellingAnnotation(problem), new Position(
					problem.getOffset(), problem.getLength()));
		}

		private void internalAccept(int start) {
			// TODO: More and better strategies
			if (start > 0 && "\\".equals(fDocument.get(start - 1, 1)))
				throw new SoftException(null);
			// return;
			if (start > 5 && "\\end{".equals(fDocument.get(start - 5, 5)))
				throw new SoftException(null);
			// return;
			if (start > 7 && "\\begin{".equals(fDocument.get(start - 7, 7)))
				throw new SoftException(null);
			// return;
		}
		...
		public void endCollecting() {
			...
			synchronized (fLockObject) {
				Iterator<Annotation> iter = fAnnotationModel
				...
				Annotation[] annotationsToRemove = (Annotation[]) toRemove
						.toArray(new Annotation[toRemove.size()]);

				if (fAnnotationModel instanceof IAnnotationModelExtension)
					((IAnnotationModelExtension) fAnnotationModel)
							.replaceAnnotations(annotationsToRemove,
									fAddAnnotations);
				else {
					for (int i = 0; i < annotationsToRemove.length; i++)
						fAnnotationModel
								.removeAnnotation(annotationsToRemove[i]);
					for (iter = fAddAnnotations.keySet().iterator(); iter
							.hasNext();) {
						Annotation annotation = (Annotation) iter.next();
						fAnnotationModel.addAnnotation(annotation,
								(Position) fAddAnnotations.get(annotation));
					}
				}
			}
	 ...
	 */
	public TeXSpellingReconcileStrategy(ISourceViewer viewer,
