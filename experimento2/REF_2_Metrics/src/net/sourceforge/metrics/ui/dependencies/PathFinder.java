...
public class PathFinder {
	 ...
	 * component used to create this TangleAnalyzer, otherwise no guarantee
	 * exists that a path can be found. Since all vertices in a strong component
	 * are reachable from each other, there must be a shortest path from every
	 * vertex of the tangle to every other vertex of the tangle, and this method
	 * should therefore never return null. Just in case, if no path is found,
	 * this method returns null. This method implements the breadth-first search
	 * from Cormen et al page 470
	 * 
	 * @param from
	 *            starting vertex
	 * @param to
	...
	public Vertex[] findShortestPath(Vertex from, Vertex to) {
		...
		return internalFindShortestPath(from, to, p, result);
	}

	private Vertex[] internalFindShortestPath(Vertex from, Vertex to, Map p,
			List result) {
	 ...
	 */
	private void buildPath(Vertex from, Vertex to, Map p, List result)
			throws IllegalArgumentException {
		if (from == to) {
			result.add(from);
		} else {
	...
	/**
	 * Simple Queue interface derived from LinkedList to make BFS more readable
	 * and related to pseudocode in the textbook.
	 * 
	 * @author Frank Sauer
	 */
	...
	private void showUI(final boolean showReverse) {
		...
		d.syncExec(new Runnable() {
			...
			public void run() {
				...
				PathFinderDialog pfd = new PathFinderDialog(shell,
						PathFinder.this, showReverse);
				pfd.open();
			}
		});
	}
