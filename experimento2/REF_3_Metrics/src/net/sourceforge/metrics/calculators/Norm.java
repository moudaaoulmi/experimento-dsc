...
/**
 * Calculates number of overridden methods for a class. Note that if the
 * inherited method is abstract or if the method under investigation calls the
 * superclass' implementation, it is <EM>not</EM> counted. A better name for
 * this metric would be NumberOfReplaced methods. The reasons for not counting
 * these is outlined on page 68 of the Lorenz and Kidd book
...
public class Norm extends Calculator implements Constants {
	...
	public void calculate(AbstractMetricSource source) {
		...
		internalCalculate(source, iType, supers);
	}

	private void internalCalculate(AbstractMetricSource source, IType iType,
			IType[] supers) {
		int overridden = 0;
	...
	private int countMethods(IType[] supers, int overridden, ArrayList counted,
			IMethod myMethod) throws JavaModelException {
