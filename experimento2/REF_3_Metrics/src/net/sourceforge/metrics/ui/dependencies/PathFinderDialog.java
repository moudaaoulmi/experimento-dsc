...
public class PathFinderDialog extends TitleAreaDialog {
	...
	/**
	 * Open a dialog to determine the shortest path from one node to another and
	 * optionally the reverse path as well. The nodes are obtained from the
	 * PathFinder
	 * 
	 * @param parent
	...
	/**
	 * Open a dialog to determine the shortest path from one node to another and
	 * no reverse path. The nodes are obtained from the PathFinder
	 * 
	 * @param parent
	 *            parent shell for this dialog
	 * @param pf
	...
	protected Control createDialogArea(Composite parent) {
		...
		pathV = new ListViewer(c, SWT.SINGLE | SWT.BORDER | SWT.V_SCROLL
				| SWT.H_SCROLL);
		pathV.getList().setFont(font);
		GridData d = new GridData(GridData.FILL_BOTH | GridData.GRAB_HORIZONTAL
		...
		if (!showReverse) {
			...
			pathV.getList().setLayoutData(d);
			revV = new ListViewer(c, SWT.SINGLE | SWT.BORDER | SWT.V_SCROLL
					| SWT.H_SCROLL);
			d = new GridData(GridData.FILL_BOTH | GridData.GRAB_HORIZONTAL
					| GridData.GRAB_VERTICAL);
			d.heightHint = convertHeightInCharsToPixels(6);
			d.widthHint = convertWidthInCharsToPixels(COLUMN_WIDTH);
	...
	private ListViewer createListViewer(Composite parent, Composite c) {
		ListViewer v = new ListViewer(c, SWT.SINGLE | SWT.BORDER | SWT.V_SCROLL
				| SWT.H_SCROLL);
	...
	protected void fromSelected(ISelection selection) {
		if ((selection != null) && !(selection.isEmpty())) {
			from = (Vertex) ((IStructuredSelection) selection)
					.getFirstElement();
		} else
			from = null;
		updateFindButton();
	...
	private void exportButtonClicked() {
		...
		if (fileName != null) {
			...
			internalExportButtonClicked(f);

		}
	}

	private void internalExportButtonClicked(File f) {
			...
			for (int t = 0; t < to.length; t++) {
				if (f != t) {
					...
					for (int p = 0; p < path.length; p++) {
						PackageAttributes a = (PackageAttributes) path[p]
								.getAttributes();
	...
	/**
	 * Find a cycle by calculating the path from from to to and the reverse path
	 * from to to from and appending them.
	 * 
	 * @param vertex
	 * @param vertex2
	...
	protected void createButtonsForButtonBar(Composite parent) {
		findButton = createButton(parent, IDialogConstants.DETAILS_ID, "Find",
				false);
		...
		findButton.setEnabled(false);
		if (showReverse) {
			exportButton = createButton(parent, IDialogConstants.INTERNAL_ID,
					"Export All", false);
