...
//#ifdef includePIM
import lancs.mobilemedia.pim.PimController;
//#endif
...
import lancs.mobilemedia.core.ui.screens.PlayMusicScreen;
//#endif
...
//@				controller.setMediaName(selectedMediaName);
	...
	void updateMedia(MediaData media) throws InvalidMediaDataException, PersistenceMechanismException {
...
// [EF] Instead of replicating this code, I change to use the method "getSelectedImageName()". 		
		...
		//#ifdef includePIM
		PimController pimcontroller = new PimController(midlet);//, getAlbumData(), (AlbumListScreen) getAlbumListScreen());
		pimcontroller.setNextController(nextcontroller);
		canv.setCommandListener(pimcontroller);
		nextcontroller = pimcontroller;
		//#endif
    ...
    * [EF] update this method or merge with method of super class.
    ...
    private boolean goToPreviousScreen() {
	    ...
	    if (currentScreenName.equals(Constants.ALBUMLIST_SCREEN)) {
			...
			System.out.println("******@@@@@@@@@@@@@@"+getCurrentStoreName());
		    showMediaList(getCurrentStoreName(), false, false);
		    ScreenSingleton.getInstance().setCurrentScreenName(Constants.IMAGELIST_SCREEN);
		    return true;
		}
