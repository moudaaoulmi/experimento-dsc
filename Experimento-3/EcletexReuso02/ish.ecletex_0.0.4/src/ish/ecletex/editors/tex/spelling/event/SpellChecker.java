...
import ish.ecletex.editors.tex.spelling.engine.Configuration;
...
import ish.ecletex.editors.tex.spelling.util.VectorUtility;
...
public class SpellChecker {
	...
	public List getSuggestions(String word, int threshold) {
		...
		for (Enumeration e = dictionaries.elements(); e.hasMoreElements();) {
			SpellDictionary dictionary = (SpellDictionary) e.nextElement();
			VectorUtility.addAll(suggestions, dictionary.getSuggestions(word,
					threshold), false);
		}
		return suggestions;
	}
	...
	public final List checkSpelling(String word) {
		// Check the spelling of the word
		if (!isCorrect(word)) {
			if ((config.getBoolean(Configuration.SPELL_IGNOREMIXEDCASE))
				...
				if (!ignoredWords.contains(word)) {
					...
					// Is this word being automagically replaced
					if (autoReplaceWords.containsKey(word)) {
						// tokenizer.replaceWord((String)
						// autoReplaceWords.get(word));
					} else {
						// JMH Need to somehow capitalise the suggestions if
						// ignoreSentenceCapitalisation is not set to true
						// Fire the event.
						// SpellCheckEvent event = new
						// BasicSpellCheckEvent(word, getSuggestions(word,
						// config.getInteger(Configuration.SPELL_THRESHOLD)),
						// tokenizer);
						return getSuggestions(word, config
								.getInteger(Configuration.SPELL_THRESHOLD));
						// terminated = fireAndHandleEvent(tokenizer, event);
					}
				}
