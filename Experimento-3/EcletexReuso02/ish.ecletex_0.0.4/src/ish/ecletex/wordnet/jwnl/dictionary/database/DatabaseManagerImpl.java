...
public class DatabaseManagerImpl implements DatabaseManager, Createable {
	...
	private static final String LEMMA_FOR_INDEX_WORD_ID_SQL = "SELECT iw.lemma "
			+ "FROM IndexWord iw "
			+ "WHERE iw.pos = ? AND iw.index_word_id = ?";

	private static final String SYNSET_IDS_FOR_INDEX_WORD_SQL = "SELECT iws.synset_id "
			+ "FROM IndexWordSynset iws, IndexWord iw "
			+ "WHERE iws.index_word_id = iw.index_word_id AND iw.pos = ?  AND iw.lemma = ?";

	private static final String COUNT_INDEX_WORDS_SQL = "SELECT MIN(index_word_id), MAX(index_word_id) FROM indexword WHERE pos = ?";

	private static final String ALL_LEMMAS_SQL = "SELECT lemma FROM IndexWord WHERE pos = ?";

	private static final String ALL_LEMMAS_LIKE_SQL = "SELECT lemma FROM IndexWord WHERE pos = ? AND lemma LIKE ?";

	private static final String SYNSET_SQL = "SELECT is_adj_cluster, gloss FROM Synset WHERE pos = ? AND file_offset = ?";

	private static final String SYNSET_WORD_SQL = "SELECT sw.word, sw.word_index "
			+ "FROM Synset s, SynsetWord sw "
			+ "WHERE s.synset_id = sw.synset_id AND s.pos = ? AND s.file_offset = ?";

	private static final String SYNSET_POINTER_SQL = "SELECT sp.pointer_type, sp.target_offset, sp.target_pos, sp.source_index, sp.target_index "
			+ "FROM Synset s, SynsetPointer sp "
			+ "WHERE s.synset_id = sp.synset_id AND s.pos = ? AND s.file_offset = ?";

	private static final String SYNSET_VERB_FRAME_SQL = "SELECT svf.frame_number, svf.word_index "
			+ "FROM Synset s, SynsetVerbFrame svf "
			+ "WHERE s.synset_id = svf.synset_id AND s.pos = ? AND s.file_offset = ?";

	private static final String ALL_SYNSETS_SQL = "SELECT offset FROM Synset WHERE pos = ?";

	private static final String EXCEPTION_SQL = "SELECT exception FROM Exception WHERE pos = ? AND lemma = ?";

	private static final String ALL_EXCEPTIONS_SQL = "SELECT lemma FROM Exception WHERE pos = ?";
	...
	public Object create(Map params) throws JWNLException {
		...
		String userName = params.containsKey(USERNAME) ? ((Param) params
				.get(USERNAME)).getValue() : null;
		String password = params.containsKey(PASSWORD) ? ((Param) params
				.get(PASSWORD)).getValue() : null;
		return new DatabaseManagerImpl(new ConnectionManager(driverClassName,
				url, userName, password));
	}

	public Query getIndexWordSynsetsQuery(POS pos, String lemma)
	...
	public Query getIndexWordLemmasQuery(POS pos, String substring)
			throws JWNLException {
		return createPOSStringQuery(pos, "%" + substring + "%",
				ALL_LEMMAS_LIKE_SQL);
	}
	...
	public Query getRandomIndexWordQuery(POS pos) throws JWNLException {
		...
		if (minMax == null) {
			...
			minMax = internalGetRandomIndexWordQuery(pos, minMax, query);
		}
	...
	private MinMax internalGetRandomIndexWordQuery(POS pos, MinMax minMax,
			Query query) throws JWNLException {
		query.execute();
		query.getResults().next();
		minMax = new MinMax(query.getResults().getInt(1), query.getResults()
				.getInt(2));
	...
	public Query getExceptionQuery(POS pos, String derivation)
			throws JWNLException {
	...
	private Query createPOSQuery(POS pos, String sql) throws JWNLException {
		Query query = null;
		return internalCreatePOSQuery(pos, sql, query);
	}

	private Query internalCreatePOSQuery(POS pos, String sql, Query query)
			throws JWNLException {
	...
	private Query createPOSStringQuery(POS pos, String str, String sql)
			throws JWNLException {
		Query query = null;
		return internalCreatePOSStringQuery(pos, str, sql, query);
	}

	private Query internalCreatePOSStringQuery(POS pos, String str, String sql,
			Query query) throws JWNLException {
		query = _connectionManager.getQuery(sql);
		query.getStatement().setString(1, pos.getKey());
	...
	private Query createPOSOffsetQuery(POS pos, long offset, String sql)
			throws JWNLException {
		Query query = null;
		return internalCreatePOSOffsetQuery(pos, offset, sql, query);
	}

	private Query internalCreatePOSOffsetQuery(POS pos, long offset,
			String sql, Query query) throws JWNLException {
		query = _connectionManager.getQuery(sql);
		query.getStatement().setString(1, pos.getKey());
	...
	private Query createPOSIdQuery(POS pos, int id, String sql)
			throws JWNLException {
		Query query = null;
		return internalCreatePOSIdQuery(pos, id, sql, query);
	}

	private Query internalCreatePOSIdQuery(POS pos, int id, String sql,
			Query query) throws JWNLException {
		query = _connectionManager.getQuery(sql);
		query.getStatement().setString(1, pos.getKey());
