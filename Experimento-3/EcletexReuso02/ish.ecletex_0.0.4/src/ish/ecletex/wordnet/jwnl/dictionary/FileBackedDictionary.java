...
import ish.ecletex.wordnet.jwnl.data.DictionaryElementType;
import ish.ecletex.wordnet.jwnl.data.Exc;
import ish.ecletex.wordnet.jwnl.data.FileDictionaryElementFactory;
...
import ish.ecletex.wordnet.jwnl.dictionary.file_manager.FileManager;
...
public class FileBackedDictionary extends AbstractCachingDictionary {
	 ...
	 * 
	 * @see ish.ecletex.wordnet.jwnl.dictionary.file_manager.RemoteFileManager
	 */
	...
	/**
	 * Install a FileBackedDictionary from a map of parameters. The keys are
	 * chose from the static variables above.
	 */
	public void install(Map params) throws JWNLException {
		...
		FileManager manager = (FileManager) ((Param) params.get(FILE_MANAGER))
				.create();
		...
		boolean enableCaching = !params.containsKey(ENABLE_CACHING)
				|| !((Param) params.get(ENABLE_CACHING)).getValue()
	...
	public IndexWord getIndexWord(POS pos, String lemma) throws JWNLException {
		...
		if (lemma.length() > 0) {
			...
			if (word == null) {
				word = internalGetIndexWord(lemma, pos, word);
			}
		}
		return word;
	}

	private IndexWord internalGetIndexWord(String lemma, POS pos, IndexWord word)
			throws JWNLException {
		long offset = getFileManager().getIndexedLinePointer(pos,
				DictionaryFileType.INDEX, lemma.replace(' ', '_'));
		if (offset >= 0) {
	...
	private IndexWord parseAndCacheIndexWordLine(POS pos, long offset,
			String line) {
	...
	// Nao foi possivel atingir no ponto exato, entao apontei para o metodo mais
	// externo...
	...
	public Iterator getSynsetIterator(POS pos) {
		return new FileLookaheadIterator(pos, DictionaryFileType.DATA) {
			protected Object parseLine(POS pos, long offset, String line) {
				return internal(pos, offset, line);
			}
		};
	}

	private Object internal(POS pos, long offset, String line) {
	...
	private Synset getSynset(POS pos, long offset, String line)
		...
		if (synset == null) {
			synset = internalGetSynset(offset, pos, line, key, synset);
		}
		return synset;
	}

	private Synset internalGetSynset(long offset, POS pos, String line,
			POSKey key, Synset synset) throws JWNLException {
		if (line == null) {
	...
	public Exc getException(POS pos, String derivation) throws JWNLException {
		...
		if (derivation != null) {
			...
			if (exc == null) {
				long offset = -1;
				exc = internalGetException(pos, derivation, exc, offset);
			}
		}
		return exc;
	}
	...
	private abstract class FileLookaheadIterator implements Iterator {
		...
		public FileLookaheadIterator(POS pos, DictionaryFileType fileType) {
			_pos = pos;
			_fileType = fileType;
			internalFileLookaheadIterator(pos, fileType);
		}

		private void internalFileLookaheadIterator(POS pos,
		...
		/**
		 * This method can be over-ridden to remove the currently pointed-at
		 * object from the data source backing the iterator.
		 */
		public void remove() {
		}
