...
//	pointcut matchHandler(): execution(public IRegion TexPairMatcher.match(IDocument, int) );
	...
	IDocument around(IDocument doc): internalGetDocumentHandler() && args(doc,*) {
		IDocument result = doc;
		try {
			System.out.println("Antes da execução do método - "+doc);
			result = proceed(doc);
			System.out.println("Depois da execução do método - "+doc);
		} catch (SoftException e) {
			e.printStackTrace();
			System.out.println("capturada aqui em SoftException...");
		} catch (CoreException e) {
			e.printStackTrace();
			System.out.println("capturada aqui em CoreException...");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("capturada aqui em Exception...");
		} catch (Throwable e) {
			e.printStackTrace();
			System.out.println("capturada aqui em Throwable...");
		}
		System.out.println("saida do advice - "+doc);
		return result;
	}
