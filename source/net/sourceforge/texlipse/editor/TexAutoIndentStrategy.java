/*
 * $Id: TexAutoIndentStrategy.java,v 1.8 2006/10/27 16:52:51 borisvl Exp $
 *
...
public class TexAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
	...
	private void setIndetationPreferenceInfo(
		...
		indent = Boolean.parseBoolean(TexlipsePlugin
				.getPreference(TexlipseProperties.INDENTATION));
		lineLength = TexlipsePlugin.getDefault().getPreferenceStore().getInt(
				TexlipseProperties.WORDWRAP_LENGTH);
		autoItem = TexlipsePlugin.getDefault().getPreferenceStore().getBoolean(
				TexlipseProperties.TEX_ITEM_COMPLETION);
	}
	...
	private boolean needsEnd(String environment, IDocument document, int coffset) {
		...
		while (counter > 0) {
			IRegion end = LatexParserUtils.findEndEnvironment(docString,
					environment, offset + 5);
			if (end == null) {
				return true;
			}
			IRegion start = LatexParserUtils.findBeginEnvironment(docString,
					environment, offset + 7);
			if (start == null) {
				counter--;
	...
	private void smartIndentAfterNewLine(IDocument document,
		...
		String endLine = document.get(commandOffset, lineLength + lineOffset
				- commandOffset);
		...
		if ((beginIndex = LatexParserUtils.findCommand(startLine, "\\begin", 0)) != -1) {
			...
			} else if (this.tools.getEnvironment(endText).equals("description")
					&& autoItem) {
				buf.append("\\item[]");
				itemSetted = true;
				itemAtLine = document.getLineOfOffset(command.offset);
			 ...
			 */
			if (needsEnd(this.tools.getEnvironment(endText), document,
					lineOffset)) {
				buf.append("\n" + prevIndentation + "\\end{"
			...
			// (itemize, table...)
		} else if ((endIndex = LatexParserUtils.findCommand(startLine, "\\end",
				0)) != -1) {
			String endText = this.tools.getEndLine(startLine
			...
			if (Arrays.binarySearch(this.indentationItems, this.tools
				...
				String prevIndentation = this.tools.getIndentation(document,
						matchingBegin, "\\begin", this.tabWidth);
	...
	private void smartIndentAfterBrace(IDocument document,
		...
		if (text.startsWith("\\end")) {
			...
			if (Arrays.binarySearch(this.indentationItems, this.tools
				...
				StringBuffer buff = new StringBuffer(this.tools.getIndentation(
	...
	private boolean itemInserted(IDocument d, DocumentCommand c) {
		...
		return internaIitemInserted(d, c, cnt, buf, lines, prevLine);
	}

	private boolean internaIitemInserted(IDocument d, DocumentCommand c,
			int cnt, StringBuffer buf, int lines, String prevLine) {
