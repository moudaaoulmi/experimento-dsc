...
public class TexDocumentProvider extends TextFileDocumentProvider {
	...
	public IDocument getDocument(Object element) {
		...
		if (element instanceof FileEditorInput) {
			...
			if (file != null) {
				...
				IDocument aux = internalGetDocument(doc, project);
				if (aux.equals(doc))
					return doc;
				...
				if (format == null || format.length() == 0) {
					TexlipseProperties
							.setProjectProperty(
									...
									TexlipseProperties.OUTPUT_FORMAT,
									TexlipsePlugin
											.getPreference(TexlipseProperties.OUTPUT_FORMAT));
					TexlipseProperties
							.setProjectProperty(
									project,
					...
					String output = name
							...
							+ TexlipsePlugin
									.getPreference(TexlipseProperties.OUTPUT_FORMAT);
					TexlipseProperties.setProjectProperty(project,
							TexlipseProperties.OUTPUTFILE_PROPERTY, output);

					IPath path = file.getFullPath();
					String dir = path.removeFirstSegments(1)
							.removeLastSegments(1).toString();
					if (dir.length() > 0) {
						TexlipseProperties.setProjectProperty(project,
								TexlipseProperties.SOURCE_DIR_PROPERTY, dir);
						TexlipseProperties.setProjectProperty(project,
								TexlipseProperties.OUTPUT_DIR_PROPERTY, dir);
						TexlipseProperties.setProjectProperty(project,
								TexlipseProperties.TEMP_DIR_PROPERTY, dir);
					}
				}
			}
		}
	...
	private IDocument internalGetDocument(IDocument doc, IProject project) {
		if (!project.hasNature(TexlipseNature.NATURE_ID)) {
			TexlipseProjectCreationOperation.addProjectNature(project,
					new NullProgressMonitor());
		} else if (TexlipseProperties.getProjectProperty(project,
				TexlipseProperties.OUTPUT_FORMAT) == null) {
