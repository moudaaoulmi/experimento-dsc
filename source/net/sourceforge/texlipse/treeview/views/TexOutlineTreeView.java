/*
 * $Id: TexOutlineTreeView.java,v 1.7 2008/09/20 18:04:14 borisvl Exp $
 *
...
public class TexOutlineTreeView extends ViewPart implements
		ISelectionChangedListener, ISelectionProvider, IPartListener {
	...
	private void resetToolbarButtons() {
		outlineActions.get(ACTION_HIDE_SEC).setChecked(
				!filter.isTypeVisible(OutlineNode.TYPE_SECTION));
		outlineActions.get(ACTION_HIDE_SUBSEC).setChecked(
				!filter.isTypeVisible(OutlineNode.TYPE_SUBSECTION));
		outlineActions.get(ACTION_HIDE_SUBSUBSEC).setChecked(
				!filter.isTypeVisible(OutlineNode.TYPE_SUBSUBSECTION));
		outlineActions.get(ACTION_HIDE_PARAGRAPH).setChecked(
				!filter.isTypeVisible(OutlineNode.TYPE_PARAGRAPH));
		outlineActions.get(ACTION_HIDE_FLOAT).setChecked(
				!filter.isTypeVisible(OutlineNode.TYPE_ENVIRONMENT));
	}
	...
	public void selectionChanged(SelectionChangedEvent event) {
		...
		if (selection.isEmpty()) {
			...
			if (node.getIFile() != null) {
				...
				internalSelectionChanged(node, input);
			}
		}
	}

	private void internalSelectionChanged(OutlineNode node,
		...
		if (e == null)
			e = (TexEditor) cPage.openEditor(input,
