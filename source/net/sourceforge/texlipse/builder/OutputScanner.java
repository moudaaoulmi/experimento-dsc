/*
 * $Id: OutputScanner.java,v 1.4 2006/04/19 18:40:12 borisvl Exp $
 *
...
public class OutputScanner {
	 ...
	 * @param console
	 */
	public OutputScanner(InputStream in, OutputStream out, String[] trig,
			String console) {
	...
	public boolean scanOutput() {
		...
		while (true) {
			...
			// TriggerStrings can only occur if the program is waiting for input
			// => in.available() == 0
			...
			if (consoleOutput != null && (char) nextByte == '\n') {
				...
				// don't print the whole buffer as the printToConsole() outputs
				// a linefeed
				BuilderRegistry.printToConsole(consoleOutput + "> "
						+ sb.substring(startOfLine, sb.length() - lf));
	...
	private static InputQueryDialog createQueryDialog(String message) {
		return InputQueryDialog.createQuery(
				"Question from an external program",
	...
	private void internalAskUserInput() {
		if (query == null) {
			query = "";
		}
