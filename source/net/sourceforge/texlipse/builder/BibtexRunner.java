...
public class BibtexRunner extends AbstractProgramRunner {
	...
	public String getArguments(IResource resource) {
		...
		if (bPos >= 0) {
			args = args.substring(0, bPos + baseName.length())
					+ args.substring(bPos + baseName.length() + 1
							+ getInputFormat().length());
		}
		return args;
		// String args = "";
	...
	private String getIncludeDirArguments(IResource project) {
		...
		String bibDirs = TexlipsePlugin.getDefault().getPreferenceStore()
				.getString(TexlipseProperties.BIB_DIR);
		if (bibDirs == null) {
			bibDirs = "";
		}

		// TODO useless? shouldn't be such a project property
		String aDir = TexlipseProperties.getProjectProperty(project,
		...
		String[] dirs = (bibDirs + aDir + File.pathSeparatorChar + pDir)
		...
		while (st.hasMoreTokens()) {
			...
			if (line.indexOf("I was expecting a ") == 0) {
				...
				if (nextLine.startsWith("--line ")) {
					...
					lineNumber = internalParseErrors(lineNumber, nextLine,
							index);
					...
					createMarker(bibResource, lineNumber, message,
							IMarker.SEVERITY_WARNING);
					createMarker(bibResource, lineNumber,
							nextLine.substring(9), IMarker.SEVERITY_WARNING);
				...
				createMarker(mainFileResource, new Integer(0),
						"Could not open bibtex database file " + bibName);
	...
	private Integer internalParseErrors(Integer lineNumber, String nextLine,
			int index) {
	...
	private void parseErrorLine(IContainer sourceDir, String line) {
		...
		Integer lineNumber = null;
		lineNumber = internalParseErrorLine(lineNumber, lineNumberString);
	...
	private Integer internalParseErrorLine(Integer lineNumber,
