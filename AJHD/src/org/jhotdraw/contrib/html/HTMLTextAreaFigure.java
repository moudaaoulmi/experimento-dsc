 ...
 * residing on external Web sources. But don't forget that this is <b>NOT</b> a
 * Web browser, so HTML contents cannot be standard Web pages with headers,
 * stylesheets, javascript and who knows what else, just plain down to earth
 * HTML code.
 * <p>
 ...
 * general is responsible for the whole display. This setting can be dynamically
 * toggled as needed.<br>
 * Note that JLabel resets the font to its own default whenever it encounters an
 * HTML structure, like a table or a header tag. I couldn't find a workaround
 ...
 * entities using the standard SGML entity notation, ex:
 * <code>&FillColor;</code>.<br>
 * Any attribute associated to the figure can be used and will be replaced with
 * an appropriate value from a ContentsProducer (see below) or its toString()
 * value if no specific ContentProducer is defined.
 * <p>
 * 
 * The HTML display and layouting can be time consuming, quite fast in most
 * cases, unless the HTML structure is complicated. This can become a serious
 * penalty when working with a large number of complicated figures.<br>
 * To help in this issue HTMLTextAreaFigure offers two display modes,
 * DirectDraw, where the HTML layout logic is executed every time the figure is
 * displayed, and BufferedDraw, where HTMLTextAreaFigure creates an in-memory
 * image of the resulting layout and uses the image for fast display until a
 * change requires to regenerate the image.<br>
 ...
 * wherever needed as you would for a Web page, then each figure using the
 * template associates the field values as attributes of the figure. The
 * attribute exposure feature will substitute the entity names with the current
 ...
 * As stated above, entities referenced in the HTML template code are replaced
 * by their current value in the drawn figure. The values themselves are
 * provided by ContentProducers.<br>
 * For a detailed description of ContentProducers please refer to their own
 * documentation, but to make it simple, a ContentProducer is an object that
 ...
 * needs the entity's value, the producer's getContent() method is called and
 * the returned value (ex: contents from a Web page, FTP file or disk file) is
 * used to replace the entity in the displayed figure.<br>
 ...
 * TextAreaFigure and HTMLTextAreaFigure classes. That's right, you can embed a
 * TextAreaFigure or HTMLTextAreaFigure contents inside an HTMLTextAreaFigure
 * recursively for as many levels as your CPU and memory will allow.<br>
 * For instance, the main figure can consists of an HTML table where each cell's
 * contents come from a different HTMLTextAreaFigure.
 * 
 ...
 */
public class HTMLTextAreaFigure extends TextAreaFigure implements
		HTMLContentProducerContext, FigureChangeListener {
	...
	static {
		fDefaultContentProducers.registerContentProducer(TextAreaFigure.class,
				new TextHolderContentProducer());
		fDefaultContentProducers.registerContentProducer(Color.class,
				new HTMLColorContentProducer());
	}
	...
	/**
	 * True if the figure contains the point. The call is relayed to the frame
	 * figure
	 * 
	 * @param x
	...
	/**
	 * Draws the figure in the given graphics. Draw is a template method calling
	 * drawBackground followed by drawText then drawFrame.<br>
	 * HTMLTextAreaFigure displays in a different order tahn most figures to
	 * avoid smearing of the border when enclosed in a weird frame figure.<br>
	 * Also, there is no such thing as a transparent background so we always
	 * draw it.
	 * 
	 * @param g
	 *            Description of the Parameter
	 * @todo check possibility of clipping the contents from the background to
	 *       have a transparent figure
	 */
	 ...
	 * 
	 * @param g
	 *            The graphics to use for the drawing
	 */
	...
	/**
	 * Draws the background for the figure. It gets the shape of the frame from
	 ...
	 * 
	 * @param g
	 *            the graphics for the drawing. It can be null when called just
	 *            to compute the size
	 * @param displayBox
	 *            the display box within which the text should be formatted and
	 *            drawn
	 * @return Description of the Return Value
	 */
	 ...
	 * Rendering involves entity substitution and HTML contents preparation
	 * suitable for display by a JLabel.
	 * 
	 * @param g2
	 *            Description of the Parameter
	 * @param drawingBox
	 *            Description of the Parameter
	 * @return Description of the Return Value
	 * @todo look for other HTML display providers as JLabel is kind of lousy at
	 *       it
	 */
	protected float renderText(Graphics2D g2, Rectangle drawingBox) {
		...
		g2.clearRect(drawingBox.x, drawingBox.y, drawingBox.width,
				drawingBox.height);
		g2.fillRect(drawingBox.x, drawingBox.y, drawingBox.width,
				drawingBox.height);
		...
		if (isRawHTML()) {
			...
			text = getHTMLText(getText(), getFont(),
					(String) getContentProducer(Color.class).getContent(this,
							FigureAttributeConstant.TEXT_COLOR_STR,
							getTextColor()), (String) getContentProducer(
							Color.class).getContent(this,
							FigureAttributeConstant.FILL_COLOR_STR,
							getFillColor()), drawingBox);
		}
		...
		displayDelegate
				.setHorizontalAlignment(((Integer) getAttribute(FigureAttributeConstant.XALIGNMENT))
						.intValue());
		displayDelegate
				.setVerticalAlignment(((Integer) getAttribute(FigureAttributeConstant.YALIGNMENT))
						.intValue());

		// finally display it
		SwingUtilities.paintComponent(g2, displayDelegate, getContainerPanel(
				displayDelegate, drawingBox), drawingBox.x, drawingBox.y,
	...
	protected Rectangle makeDrawingBox(Rectangle displayBox) {
		...
		float leftMargin = ((Float) getAttribute(FigureAttributeConstant.LEFT_MARGIN))
				.floatValue();
		float rightMargin = ((Float) getAttribute(FigureAttributeConstant.RIGHT_MARGIN))
				.floatValue();
		float topMargin = ((Float) getAttribute(FigureAttributeConstant.TOP_MARGIN))
				.floatValue();
		float bottomMargin = ((Float) getAttribute(FigureAttributeConstant.BOTTOM_MARGIN))
				.floatValue();
	...
	protected void createImage(int width, int height) {
		...
		if (!fImageHolder.isAvailable()
			...
			fImageHolder.setResource(new BufferedImage(width, height,
					BufferedImage.TYPE_INT_RGB));
		}
		// we don't unlock the image, it's the responsibility of
		// the caller to do so, this in oreder to ensure that calling
		// createImage
	 ...
	 * 
	 * @param drawingDelegate
	 *            The delegate
	 * @param displayBox
	 *            The bounding box
	 * @return The container
	 */
	protected JPanel getContainerPanel(Component drawingDelegate,
	 ...
	 *            The font
	 * @param textColor
	 *            The text color HTML code
	 * @param backColor
	 *            The background's color HTML code
	 * @param displayBox
	 *            Description of the Parameter
	...
	protected String getHTMLText(String text, Font font, String textColor,
		...
		// a single cell
		htmlText.append("<table border='0' width='" + displayBox.width
		...
		// set the font
		htmlText
				.append("<font face='&FontName;' color='&TextColor;' size='&FontSize;'>");
		// add alignment if required
		if (((Integer) getAttribute(FigureAttributeConstant.XALIGNMENT))
				.intValue() == SwingConstants.CENTER) {
	...
	/**
	 * Returns a new String with the entity keywords replaced by their current
	 * attribute value.<br>
	 * The text is scanned for entity keywords delimited by the
	 * START_ENTITY_CHAR and END_ENTITY_CHAR characters as in<br>
	...
	private int substituteEntityKeywordsPartOne(String template,
		...
		chunkEnd = internalSubstituteEntityKeywordsPartOne(template, finalText, startPos, chunkEnd);
		return chunkEnd;
	}

	private int internalSubstituteEntityKeywordsPartOne(String template, StringBuffer finalText, int startPos,
			int chunkEnd) {
		int endPos;
	...
	protected String getEntityHTMLRepresentation(String attrName) {
		...
		Object attrValue = getIntrinsicContentProducer().getContent(this,
				attrName, null);
		...
		while (attrValue != null && !(attrValue instanceof String)) {
			...
			if (attrValue instanceof ContentProducer) {
				attrValue = ((ContentProducer) attrValue).getContent(this,
						attrName, attrValue);
				continue;
			}

			// not a specific producer, try a default one
			ContentProducer defaultProducer = getContentProducer(attrValue
					.getClass());
			if (defaultProducer != null) {
				attrValue = defaultProducer.getContent(this, attrName,
						attrValue);
				continue;
			}
	 ...
	 * 
	 * @param popupMenu
	 *            The popup menu to add items to
	 */
	protected void addPopupMenuItems(JPopupMenu popupMenu) {
		...
		rbOption = new JRadioButtonMenuItem(new AbstractAction(
				"Buffered drawing") {
			public void actionPerformed(ActionEvent event) {
	 ...
	 * 
	 * @return True if currently doing direct drawing, ie: not using a cached
	 *         image
	 */
	 ...
	 * 
	 * @param newUseDirectDraw
	 *            The new useDirectDraw value
	 */
	 ...
	 * 
	 * @param newUseBufferedDraw
	 *            The new useBufferedDraw value
	 */
	 ...
	 * "BottomMargin", "TabSize", "FontSize", "FontStyle", and "FontName"
	 * attributes
	 * 
	 * @param name
	 ...
	 * In RawHTML mode, the figure does not add any HTML formatting information
	 * so it's the user's responsibility to handle the whole displayed contents
	 * 
	 ...
	 * object.<br>
	 * The intrinsic producer produces contents for the basic figure's
	 * attributes
	 * 
	...
	/**
	 * Sets the IntrinsicContentProducer attribute of the HTMLTextAreaFigure
	 * object
	 * 
	 * @param newIntrinsicContentProducer
	 *            The new IntrinsicContentProducer value
	 */
	public void setIntrinsicContentProducer(
			ContentProducer newIntrinsicContentProducer) {
	 ...
	 */
	public ContentProducer registerContentProducer(Class targetClass,
	 ...
	 *            Description of the Parameter
	 */
	public void unregisterContentProducer(Class targetClass,
			ContentProducer producer) {
	...
	public Polygon getPolygon() {
		...
		FlatteningPathIterator pIter = new FlatteningPathIterator(
				getClippingShape().getPathIterator(at), 1);
	 ...
	 * 
	 * @param newFrameFigure
	 *            The new frameFigure value
	 */
	...
	public void figureChanged(FigureChangeEvent e) {
		willChange();
		super.basicDisplayBox(e.getFigure().displayBox().getLocation(), Geom
				.corner(e.getFigure().displayBox()));
		changed();
	}
	...
	/**
	 * Usually, a TextHolders is implemented by a Figure subclass. To avoid
	 * casting a TextHolder to a Figure this method can be used for polymorphism
	 * (in this case, let the (same) object appear to be of another type). Note,
	 * that the figure returned is not the figure to which the TextHolder is
	 * (and its representing figure) connected.
	 * 
	 * @return figure responsible for representing the content of this
	 *         TextHolder
	 */
