...
public class SpellDictionaryDisk extends SpellDictionaryASpell {
	 ...
	 * <code>File</code> unless you are sure that a new dictionary does not have
	 * to be created. In the future, some sort of external locking mechanism may
	 * be created that handles this scenario gracefully.
	 * 
	 * @param base
	 *            the base directory in which <code>SpellDictionaryDisk</code>
	 *            can expect to find its necessary files
	 * @param block
	 *            if a new word db needs to be created, there can be a
	...
	public List getWords(String code) {
		...
		if (posLen != null) {
			internalGetWords(code, words, posLen);
		}

		return words;
	}

	private void internalGetWords(String code, Vector words, int[] posLen) {
	...
	private boolean newDictionaryFiles() throws FileNotFoundException,
		...
		if (c.exists()) {
			...
			internalNewDictionaryFiles(contents, c, reader);
		}
		...
		if (contents.size() != wordFiles.length) {
			...
			for (int i = 0; i < wordFiles.length; i++) {
				FileSize fs = new FileSize(wordFiles[i].getName(), wordFiles[i]
						.length());
	...
	private void internalNewDictionaryFiles(List contents, File c,
			BufferedReader reader) throws FileNotFoundException, IOException {
