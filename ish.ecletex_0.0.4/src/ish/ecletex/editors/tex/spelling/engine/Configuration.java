...
public abstract class Configuration {
	...
	/**
	 * used by EditDistance: the cost of having to swap two adjoinging
	 * characters for the swap value to ever be used, it should be smaller than
	 * the COST_REMOVE_CHAR or COST_INSERT_CHAR values <br/>
	 * (integer greater than 0)
	 */
	...
	/**
	 * the first word of a sentance is expected to start with an upper case
	 * letter <br/>
	 * (boolean)
	 */
	...
	public static final Configuration getConfiguration(String className) {
		...
		if (className != null && className.length() > 0) {
			result = internalGetConfiguration(className, result);
		} else {
	...
	private static Configuration internalGetConfiguration(String className,
