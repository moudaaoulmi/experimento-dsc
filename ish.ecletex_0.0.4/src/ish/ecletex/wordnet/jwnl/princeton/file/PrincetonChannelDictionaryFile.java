 ...
 * Princeton's dictionary file naming convention. Uses
 * java.nio.channels.FileChannel for file access.
 */
public class PrincetonChannelDictionaryFile extends
	...
	public PrincetonChannelDictionaryFile(String path, POS pos,
			DictionaryFileType fileType) {
	...
	public String readLine() throws IOException {
		if (isOpen()) {
			// The following lines gratuitously lifted from
			...
			return ((c == -1) && (input.length() == 0)) ? null : input
					.toString();
		} else {
	...
	protected void openFile(File file) throws IOException {
		...
		_buffer = Charset.forName("ISO-8859-15").newDecoder()
				.decode(
						_channel.map(FileChannel.MapMode.READ_ONLY, 0, _channel
								.size()));
	}
