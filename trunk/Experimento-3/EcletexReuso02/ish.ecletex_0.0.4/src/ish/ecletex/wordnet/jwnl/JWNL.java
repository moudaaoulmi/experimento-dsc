...
public final class JWNL {
	...
	public static void initialize(InputStream propertiesStream)
		...
		_initStage = START;
		internalInitialize(propertiesStream);

		// parse the properties file
		Document doc = null;
		doc = internalInitialize(propertiesStream, doc);

		// do this in a separate try/catch since parse can also throw an
		// IOException
		internalInitialize1(propertiesStream);
		...
		for (int i = 0; i < resourceNodes.getLength(); i++) {
			String resource = getAttribute(resourceNodes.item(i),
					CLASS_ATTRIBUTE);
			if (resource != null) {
	...
	private static void internalInitialize1(InputStream propertiesStream) {
		propertiesStream.close();
	}

	private static Document internalInitialize(InputStream propertiesStream,
			Document doc) throws JWNLException {
		DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		factory.setValidating(false);
		DocumentBuilder docBuilder = factory.newDocumentBuilder();
		return docBuilder.parse(propertiesStream);
	}

	private static void internalInitialize(InputStream propertiesStream)
			throws JWNLException {
	...
	private static Element createElementFromNode(Node node)
			throws JWNLException {
		return new Element(getAttribute(node, CLASS_ATTRIBUTE), getParams(node
				.getChildNodes()));
	}
		...
		for (int i = 0; i < list.getLength(); i++) {
			...
			if (n.getNodeType() == Node.ELEMENT_NODE
				...
				if (name == null && value == null) {
					...
					Param param = null;
					if (value == null) {
						param = new ParamList(name.toLowerCase(), getParams(n
								.getChildNodes()));
					} else if (name == null) {
						param = new ValueParam(value, getParams(n
								.getChildNodes()));
					} else {
	...
	public static final class OS {
		...
		/**
		 * Returns true if <var>testOS</var> is a version of this OS. For
		 * example, calling WINDOWS.matches("Windows 95") returns true.
		 */
	...
	public static final class Version {
		...
		public String toString() {
			return resolveMessage("JWNL_TOSTRING_002", new Object[] {
					_publisher, new Double(_number), _locale });
		}
