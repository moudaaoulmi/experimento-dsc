...
/**
 * <code>ObjectDictionaryFile</code> that accesses files names with the Princeton
 * dictionary file naming convention.
 */
public class PrincetonObjectDictionaryFile extends
		AbstractPrincetonDictionaryFile implements ObjectDictionaryFile {
	...
	public PrincetonObjectDictionaryFile(String path, POS pos,
			DictionaryFileType fileType) {
	...
	/**
	 * Here we try to be intelligent about opening streams. If the file does not
	 * already exist, we assume that we are going to be creating it and writing
	 * to it, otherwise we assume that we are going to be reading from it. If
	 * you want the other stream open, you must do it explicitly by calling
	 * <code>openStreams</code>.
	 */
