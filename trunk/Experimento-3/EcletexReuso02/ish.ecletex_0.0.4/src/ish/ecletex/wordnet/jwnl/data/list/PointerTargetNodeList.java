...
import ish.ecletex.wordnet.jwnl.util.TypeCheckingList;
...
/**
 * A <code>PointerTargetNodeList</code> holds the results of a relationship
 * method. Each node contains a <code>PointerTarget</code> (a synset or word)
 * and the type of relationship that the node has to the other elements in the
 * list and/or to the source word.
 */
public class PointerTargetNodeList extends TypeCheckingList {
	private static final NodePrinter PRINTER = new NodePrinter(System.out, 2) {
		public void print(PrintStream stream, Node node, int indent,
				int indentIncrement) {
	...
	public PointerTargetTreeNodeList toTreeList() {
		...
		while (itr.hasNext()) {
			...
			list.add(new PointerTargetTreeNode(node.getPointerTarget(), node
					.getType()));
		}
		return list;
	}
	...
	/**
	 * Reverse the contents of this list. This function creates a copy of this
	 * list and reverses it, so there are no changes made to this list itself.
	 */
