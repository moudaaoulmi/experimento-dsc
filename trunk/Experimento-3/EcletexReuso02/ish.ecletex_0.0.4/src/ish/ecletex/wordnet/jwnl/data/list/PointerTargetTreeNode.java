...
import ish.ecletex.wordnet.jwnl.data.PointerTarget;
...
/**
 * A node in a <code>PointerTargetTreeNodeList</code>. Each node can have a
 * childTreeList, which is a list of nodes that are children of this node, and a
 * pointerTreeList, which is a tree of pointers related to this node's target.
 * Basically, this allows for a single tree supporting multiple relationships.
 * For example, you may have a ancestry tree, and each node, besides having
 * links to its children, has links to the synonyms of its target.
 */
public class PointerTargetTreeNode extends PointerTargetNode {
	...
	public PointerTargetTreeNode(PointerTarget target,
			PointerTargetTreeNodeList childTreeList, PointerType type) {
	...
	public String toString() {
		if (_cachedToString == null) {
			_cachedToString = JWNL.resolveMessage("DATA_TOSTRING_015",
					new Object[] { getPointerTarget(), getType(),
	...
	/**
	 * Convert this node into a list of PointerTargetList's, each representing a
	 * unique brance through the tree
	 */
	public List toList(PointerTargetNodeList list) {
		...
		if (hasValidChildTreeList()) {
			...
			while (itr.hasNext()) {
				l.addAll(((PointerTargetTreeNode) itr.next())
						.toList((PointerTargetNodeList) list.clone()));
			}
		} else {
			l.add(list);
		}
	...
	public Object clone() {
		return new PointerTargetTreeNode(getPointerTarget(),
				getChildTreeList(), getPointerTreeList(), getType(),
				getParent());
	}
