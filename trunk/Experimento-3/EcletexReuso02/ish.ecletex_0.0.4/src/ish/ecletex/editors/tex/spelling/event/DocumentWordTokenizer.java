...
public class DocumentWordTokenizer implements WordTokenizer {
	...
	public DocumentWordTokenizer(Document document) {
		...
		internalDocumentWordTokenizer(document);
	}

	private void internalDocumentWordTokenizer(Document document) {
	...
	private String internalNextWord(String word) {
		word = document
				.getText(currentWordPos, currentWordEnd - currentWordPos);
		return word;
	}
	...
	public void replaceWord(String newWord) {
		if (currentWordPos != -1) {
			internalReplaceWord(newWord);
			// Position after the newly replaced word(s)
			first = true;
			currentWordPos = getNextWordStart(text, currentWordPos
					+ newWord.length());
			if (currentWordPos != -1) {
				currentWordEnd = getNextWordEnd(text, currentWordPos);
				nextWordPos = getNextWordStart(text, currentWordEnd);
	...
	private void internalReplaceWord(String newWord) {
