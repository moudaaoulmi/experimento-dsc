...
import ish.ecletex.editors.bibtex.dom.BibtexAbstractValue;
 ...
 * different Expanders defined in the ish.ecletex.editors.bibtex.expansions
 * package if you need more than that.
 * 
 * @author henkel
 */
public final class BibtexParser {
	...
	public void parse(BibtexFile bibtexFile, Reader input)
	...
	private void internalParse() throws IOException {
		parseEntry();
	}

	private final static char[] EXCEPTION_SET_NAMES = new char[] { '"', '#',
			'%', '\'', '(', ')', ',', '=', '{', '}' };
	private final static String[] ENTRY_TYPES = new String[] { "string",
			"preamble", "article", "book", "booklet", "conference", "inbook",
	...
	private void parseEntry() throws ParseException, IOException {
		...
		final int bracketChoice = lexer.scanAlternatives(
				new char[] { '{', '(' }, false);

		if (entryType.equals("string")) {
			String stringName = lexer.scanLiteral(EXCEPTION_SET_NAMES, true,
					true);
			lexer.scan('=');
			BibtexAbstractValue value = parseValue();
			bibtexFile.addEntry(bibtexFile.makeMacroDefinition(stringName,
					value));
			...
			String bibkey = (lexer.currentInputChar() == ',') ? "" : lexer
					.scanLiteral(new char[] { ',' }, true, true);
			final BibtexEntry entry = bibtexFile.makeEntry(entryType, bibkey);
			bibtexFile.addEntry(entry);
			while (true) {
				lexer
						.enforceNoEof("',' or corresponding closing bracket",
								true);
				...
				if (lexer.currentInputChar() == ',') {
					...
					if (lexer.currentInputChar() == '}')
						break;
					String fieldName = lexer.scanLiteral(EXCEPTION_SET_NAMES,
							true, true);
					lexer.scan('=');
					BibtexAbstractValue value = parseValue();
	...
	private BibtexAbstractValue parseValue() throws ParseException, IOException {
		...
		if (inputCharacter == '"') {
			...
			String stringContent = lexer.scanLiteral(EXCEPTION_SET_NAMES,
					false, true).trim();
