...
public class LackOfCohesion extends Calculator implements Constants {
	...
	public void calculate(AbstractMetricSource source)
		...
		internalCalculate(source);
	}

	private void internalCalculate(AbstractMetricSource source) {
	...
	private void visitMethods(IMethod[] methods) {
		...
		for (int i = 0; i < methods.length; i++) {
			...
			internalVisitMethods(methods, countStatics, i, methodName);
		}
	}

	private void internalVisitMethods(IMethod[] methods, boolean countStatics,
			int i, String methodName) {
	...
	private void initBuckets(IField[] fields) {
		...
		internalInitBuckets(fields);
	}

	private void internalInitBuckets(IField[] fields) {
