 ...
 *
 *
 * $Id: CompilationUnitMetrics.java,v 1.40 2004/04/30 03:33:24 sauerf Exp $
 */
...
public class CompilationUnitMetrics extends AbstractMetricSource {
		...
		for (Iterator i = types.iterator(); i.hasNext();) {
			...
			if (!lastType.isInterface()) {
				...
				TypeMetrics tm = (TypeMetrics) Dispatcher
						.calculateAbstractMetricSource(type, this, data);
				addChild(tm);
	...
	private void internalInitializeChildren(ICompilationUnit unit) {
		unit.getUnderlyingResource().deleteMarkers(
				"net.sourceforge.metrics.outofrangemarker", true,
				IResource.DEPTH_INFINITE);
	}

	// nao consegui recuperar o valor do inteiro corretamente
		...
		try {
			...
			for (int j = 0; j < members.length; j++) {
				if (!memberDeclarations[j].isInterface()) {
					...
					TypeMetrics inner = (TypeMetrics) Dispatcher
							.calculateAbstractMetricSource(members[j], this,
									data);
					addChild(inner);
				} else
					interfaces++;
