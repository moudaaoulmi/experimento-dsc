...
public class ScreenFlowManager implements java.io.Serializable {
	...
	public void init(ServletContext context) {
		...
		requestMappingsURL = internalInit(context, requestMappingsURL);
		urlMappings = (HashMap) context.getAttribute(WebKeys.URL_MAPPINGS);
		ScreenFlowData screenFlowData = URLMappingsXmlDAO
	...
	private String internalInit(ServletContext context,
			String requestMappingsURL) {
		requestMappingsURL = context.getResource("/WEB-INF/mappings.xml")
				.toString();
		return requestMappingsURL;
	}
	...
	public void forwardToNextScreen(HttpServletRequest request,
		...
		if (urlMapping != null) {
			if (!urlMapping.useFlowHandler()) {
				...
				currentScreen = internalForwardToNextScreen(request,
						currentScreen, urlMapping, flowHandlerString);
			}
		}
	...
	private String internalForwardToNextScreen(HttpServletRequest request,
			String currentScreen, URLMapping urlMapping,
			String flowHandlerString) {
		FlowHandler handler;
		handler = (FlowHandler) getClass().getClassLoader().loadClass(
				flowHandlerString).newInstance();
