...
/**
 * Provides all the startup objects needed by the WAF which will be loaded when
 * the application Servlet Context is started
 */
public class ApplicationComponentManager implements ServletContextListener {
	...
	private void doInit(ServletContext context) {
		...
		requestMappingsURL = internalDoInit(context, requestMappingsURL);
		HashMap urlMappings = URLMappingsXmlDAO
				.loadRequestMappings(requestMappingsURL);
		context.setAttribute(WebKeys.URL_MAPPINGS, urlMappings);
		HashMap eventMappings = URLMappingsXmlDAO
				.loadEventMappings(requestMappingsURL);
		context.setAttribute(WebKeys.EVENT_MAPPINGS, eventMappings);
	}

	private String internalDoInit(ServletContext context,
