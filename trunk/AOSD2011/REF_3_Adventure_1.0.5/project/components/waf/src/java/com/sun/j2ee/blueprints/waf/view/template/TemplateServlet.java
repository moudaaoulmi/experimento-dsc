...
public class TemplateServlet extends HttpServlet {
	...
	private void initScreens(ServletContext context, String language) {
		...
		screenDefinitionURL = internalInitScreens(context, language,
				screenDefinitionURL);
		if (screenDefinitionURL != null) {
			...
			if (screenDefinitions != null) {
				...
				System.err
						.println("Template Servlet Error Loading Screen Definitions: Confirm that file at URL /WEB-INF/screendefinitions_"
								+ language
								+ ".xml contains the screen definitions");
			}
		} else {
			System.err
					.println("Template Servlet Error Loading Screen Definitions: URL /WEB-INF/screendefinitions_"
							+ language + ".xml not found");
		}
	}

	private URL internalInitScreens(ServletContext context, String language,
			URL screenDefinitionURL) {
		screenDefinitionURL = context.getResource("/WEB-INF/screendefinitions_"
				+ language + ".xml");
		return screenDefinitionURL;
	}
	...
	public void process(HttpServletRequest request, HttpServletResponse response)
		...
		if (languageParam != null) {
			...
			localeString = ((Locale) request.getSession().getAttribute(
					WebKeys.LOCALE)).toString();
		}
		...
		if (screenName.equals(PREVIOUS_SCREEN)) {
			...
			String extension = selectedUrl.substring(lastDot, selectedUrl
					.length());
		...
		if (screenName != null) {
			...
			if (screen != null) {
				...
				if (templateName != null) {
					...
					out
							.println("<font size=+4>Error:</font><br>Template file not found for "
									+ screenName);
				}
			} else {
				...
				PrintWriter out = response.getWriter();
				out
						.println("<font size=+4>Error:</font><br>Definition for screen "
