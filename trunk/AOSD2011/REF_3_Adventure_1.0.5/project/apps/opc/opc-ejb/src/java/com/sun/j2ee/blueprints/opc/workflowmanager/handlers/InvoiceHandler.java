...
/**
 * This is the Invoice handler that gets called by the OPC Work Flow Manager.
 * This handler calls the workers that process the invoice
 */
public class InvoiceHandler {
	...
	public void handle(Message message) throws HandlerException {
		...
		internalHandle(message, invoice, inv, opcPoID);
	}

	private void internalHandle(Message message, Invoice invoice, String inv,
			String opcPoID) throws HandlerException {
	...
	public void sendOrderCompletedMail(String orderID) throws HandlerException {
		...
		internalSendOrderCompletedMail(orderID, sendMail);

	}

	private void internalSendOrderCompletedMail(String orderID, boolean sendMail)
			throws HandlerException {
		// get email id and call CRM
		if (sendMail) {
			PurchaseOrderLocalHome poHome = (PurchaseOrderLocalHome) sl
					.getLocalHome(JNDINames.PO_EJB);
