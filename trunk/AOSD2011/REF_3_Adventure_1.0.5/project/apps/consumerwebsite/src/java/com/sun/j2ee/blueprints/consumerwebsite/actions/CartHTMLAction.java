...
public final class CartHTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if (actionType.equals("select_package")) {
			...
			internalPerform(packageId, ac, facade);
	...
	private void internalPerform(String packageId, Cart ac, CatalogFacade facade) {
	...
	private void purchaseActivities(HttpServletRequest request) {
		...
		while (it.hasNext()) {
			...
			if (key.startsWith("activity_")) {
				...
				id = id.trim();
				int qty = 0;
				qty = internalPurchaseActivities(value, qty);
				if (qty > 0) {
	...
	private int internalPurchaseActivities(String value, int qty) {
		if (value != null)
			qty = (new Integer(value)).intValue();
		return qty;
	}
	...
	private void updatePackage(HttpServletRequest request) {
		...
		if (lodgingRoomCountString != null) {
			int lodingRoomCount = (new Integer(lodgingRoomCountString))
					.intValue();
	...
	private void getResultBean(HttpServletRequest request)
		...
		CatalogFacade facade = acm.getCatalogFacade(session);
		// Nao pode ser extraido. Contem mais de uma dependencia
		try {
