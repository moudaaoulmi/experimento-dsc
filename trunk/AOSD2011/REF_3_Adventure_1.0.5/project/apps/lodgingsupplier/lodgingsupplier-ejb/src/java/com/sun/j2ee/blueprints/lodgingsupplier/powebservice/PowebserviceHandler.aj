...
public privileged aspect PowebserviceHandler {
	...
	pointcut internalFromXMLHandler(): execution(private LodgingOrder LodgingOrder.internalFromXML(String,LodgingOrder));

	pointcut internalSubmitRequestHandler(): execution(private void LodgingPOEndpointBean.internalSubmitRequest(LodgingOrder,ConnectionFactory , Destination , Connection , Session ,MessageProducer ));

	pointcut getConnectionHandler(): call(public Connection ConnectionFactory.createConnection()) && withincode(private void LodgingPOEndpointBean.internalSubmitRequest(LodgingOrder,ConnectionFactory , Destination , Connection , Session ,MessageProducer ));

	pointcut getSessionHandler(): call(public Session createSession(boolean , int )) && withincode(private void LodgingPOEndpointBean.internalSubmitRequest(LodgingOrder,ConnectionFactory , Destination , Connection , Session ,MessageProducer ));

	pointcut getSenderHandler(): call(public MessageProducer javax.jms.Session.createProducer(Destination)) && withincode(private void LodgingPOEndpointBean.internalSubmitRequest(LodgingOrder,ConnectionFactory , Destination , Connection , Session ,MessageProducer ));
	...
	LodgingOrder around() throws InvalidOrderException : internalFromXMLHandler() {
		try {
			return proceed();
		} catch (Exception exe) {
