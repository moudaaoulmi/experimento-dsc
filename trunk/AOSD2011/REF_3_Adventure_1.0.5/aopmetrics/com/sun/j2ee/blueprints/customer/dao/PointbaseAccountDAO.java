...
public class PointbaseAccountDAO implements AccountDAO {
	...
	private void insertAccount(Account details) throws DAOSystemException,
		...
		internalInsertAccount(details, stmt, info, dbConnection);
	}

	private void internalInsertAccount(Account details, PreparedStatement stmt,
			ContactInformation info, Connection dbConnection)
			throws DAOSystemException, AccountDAODBUpdateException {
	...
	private boolean userExists(String userId) throws DAOSystemException {
		...
		Connection dbConnection = null;
		returnValue = internalUserExists(userId, returnValue, stmt, result,
				dbConnection);
		return returnValue;
	}

	private boolean internalUserExists(String userId, boolean returnValue,
	...
	private Account selectAccount(String userId) throws DAOSystemException,
		...
		ResultSet result = null;
		Connection dbConnection = null;
		return internalSelectAccount(userId, stmt, result, dbConnection);
	}

	private Account internalSelectAccount(String userId,
			PreparedStatement stmt, ResultSet result, Connection dbConnection)
			throws DAOSystemException, AccountDAOFinderException {
		dbConnection = DAOUtils.getDBConnection(JNDINames.CUSTOMER_DATASOURCE);
