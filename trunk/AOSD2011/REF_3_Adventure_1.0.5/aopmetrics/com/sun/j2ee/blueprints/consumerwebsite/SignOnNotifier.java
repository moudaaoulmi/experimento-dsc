...
public class SignOnNotifier implements java.io.Serializable,
		...
		if (name.equals(SignOnFilter.SIGNED_ON_USER)) {
			...
			if (aSignOn) {
				...
				if (acm != null) {
					...
					bean = internalProcessEvent(session, facade, action, bean);
	...
	private CustomerBean internalProcessEvent(HttpSession session,
			CustomerFacade facade, CustomerHTMLAction action, CustomerBean bean) {
