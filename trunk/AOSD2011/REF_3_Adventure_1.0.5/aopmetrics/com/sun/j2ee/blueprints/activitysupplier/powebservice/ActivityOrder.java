...
public class ActivityOrder implements Serializable {
	...
	public static ActivityOrder fromXML(String actyPO)
			throws InvalidOrderException {

		ActivityOrder order = null;
		String opcPoId = null;
		ArrayList acts = new ArrayList();
		opcPoId = internalFromXML(opcPoId, actyPO, acts);
		if (acts.size() != 0) {
	...
	private static String internalFromXML(String opcPoId, String actyPO,
			ArrayList acts) throws InvalidOrderException {
		...
		for (elem = getNextSibling(elem); elem != null; elem = getNextSibling(elem)) {
			...
			Date date = new SimpleDateFormat("MM-dd-yy")
					.parse(((Text) (actyelem.getFirstChild())).getData());
			Calendar cal = Calendar.getInstance();
			cal.setTime(date);
			acty.setStartDate(cal);
			actyelem = getNextSibling(actyelem);
			date = new SimpleDateFormat("MM-dd-yy").parse(((Text) (actyelem
					.getFirstChild())).getData());
			cal = Calendar.getInstance();
			cal.setTime(date);
			acty.setEndDate(cal);
			actyelem = getNextSibling(actyelem);
			acty.setHeadCount(Integer.parseInt(((Text) (actyelem
					.getFirstChild())).getData()));
			acts.add(acty);
		}
		return opcPoId;
	...
	public static Element getNextSibling(Element elem) {
		for (Node sib = elem.getNextSibling(); sib != null; sib = sib
				.getNextSibling()) {
