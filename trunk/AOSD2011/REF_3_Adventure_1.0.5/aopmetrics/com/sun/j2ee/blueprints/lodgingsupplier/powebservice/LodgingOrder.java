...
public class LodgingOrder implements Serializable {
	...
	public static LodgingOrder fromXML(String lodgyPO)
		...
		lodgy = internalFromXML(lodgyPO, lodgy);
		return lodgy;
	}

	private static LodgingOrder internalFromXML(String lodgyPO,
			LodgingOrder lodgy) throws InvalidOrderException {
		...
		elem = getNextSibling(elem);
		Date date = new SimpleDateFormat("MM-dd-yy").parse(((Text) (elem
				.getFirstChild())).getData());
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		lodgy.setStartDate(cal);
		elem = getNextSibling(elem);
		date = new SimpleDateFormat("MM-dd-yy").parse(((Text) (elem
				.getFirstChild())).getData());
		cal = Calendar.getInstance();
		cal.setTime(date);
		lodgy.setEndDate(cal);
		elem = getNextSibling(elem);
		lodgy.setHeadCount(Integer.parseInt(((Text) (elem.getFirstChild()))
				.getData()));
		return lodgy;
	}
