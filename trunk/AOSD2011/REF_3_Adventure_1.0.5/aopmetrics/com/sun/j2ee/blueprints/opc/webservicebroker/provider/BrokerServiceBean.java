...
public class BrokerServiceBean implements SessionBean {
	...
	public boolean validate(String xmlText) {
		Document doc = null;
		if (internalValidate(xmlText))
			return true;
		return false;
	}

	private boolean internalValidate(String xmlText) {
		Document doc;
		DocumentBuilderFactory dbf = null;
		DocumentBuilder db = null;
		db = internalValidate(db);
		InputSource is = new InputSource(new StringReader(xmlText));
		doc = db.parse(is);
		return true;
	}

	private DocumentBuilder internalValidate(DocumentBuilder db)
