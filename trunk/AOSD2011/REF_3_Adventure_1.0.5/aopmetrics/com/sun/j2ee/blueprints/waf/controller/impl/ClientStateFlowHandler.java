...
/**
 * This class de-serializes Base64 encoded parameters encoded into a web page
 * using the ClientCacheLinkTag.
 * 
 */
public class ClientStateFlowHandler implements FlowHandler {
		...
		if (!params.isEmpty()) {
			...
			while (it.hasNext()) {
				...
				if (key.startsWith(cacheId + "_attribute_")) {
					...
					internalProcessFlow(request, cacheId, key, bytes);
				}
			}
		}
	...
	private void internalProcessFlow(HttpServletRequest request,
			String cacheId, String key, byte[] bytes) {
		ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(
				bytes));
		Object requestObject = requestObject = ois.readObject();
		ois.close();
		// put the de-serialized object back into the request
		String requestObjectKey = key.substring((cacheId + "_attribute_")
				.length(), key.length());
