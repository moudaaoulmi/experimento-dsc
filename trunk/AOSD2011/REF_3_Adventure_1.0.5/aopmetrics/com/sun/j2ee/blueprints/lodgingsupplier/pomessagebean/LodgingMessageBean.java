...
import com.sun.j2ee.blueprints.lodgingsupplier.purchaseorder.ejb.*; //import com.sun.j2ee.blueprints.opc.webservicebroker.provider.BrokerServiceIntf;
...
public class LodgingMessageBean implements MessageDrivenBean, MessageListener {
	...
	public void onMessage(Message message) {
		...
		lo = internalOnMessage(message, lo);
		internalOnMessage(lo);

	}

	private void internalOnMessage(LodgingOrder lo) {
		doWork(lo);
	}

	private LodgingOrder internalOnMessage(Message message, LodgingOrder lo) {
	...
	public void doWork(LodgingOrder lodge) throws OrderSubmissionException {
		internalDoWork(lodge);
		sendInvoice(lodge);
	}

	private void internalDoWork(LodgingOrder lodge) {
		persistOrder(lodge);
	}
