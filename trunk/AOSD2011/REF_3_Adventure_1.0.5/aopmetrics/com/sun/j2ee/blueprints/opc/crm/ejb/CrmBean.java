...
public class CrmBean implements MessageDrivenBean, MessageListener {
	...
	public void onMessage(Message recvMsg) {
		...
		String result = null;
		internalOnMessage(recvMsg, recdText);
	}

	private void internalOnMessage(Message recvMsg, String recdText)
			throws EJBException {
		TextMessage recdTM;
		String result;
	...
	private String doWork(String message) throws JMSException {
		...
		if (recipient == null)
			return "failed";
		internalDoWork(recipient, xmlMessage, title);
		return "success";
	}

	private void internalDoWork(String recipient, String xmlMessage,
			String title) {
	...
	private Document getDocument(String xmlText) {
		...
		db = internalGetDocument(db);
		InputSource is = new InputSource(new StringReader(xmlText));
		doc = db.parse(is);
		return doc;
	}

	private DocumentBuilder internalGetDocument(DocumentBuilder db) {
