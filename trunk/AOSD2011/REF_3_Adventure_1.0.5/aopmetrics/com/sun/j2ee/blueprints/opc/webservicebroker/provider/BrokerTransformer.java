...
public class BrokerTransformer {
	...
	/**
	 * The InputSource provided to this class needs to be read 2 times. We need
	 * to read the source to determine what document transformation needs to be
	 * perfomed (e.g. which style sheet to apply) and then we need to transform
	 * the actuall document. In order to do this we coppy the InputSource to a
	 * ByteArrayOutputStream so that we can create a new InputSource whenever we
	 * need to do something with the doucment.
	 */

	public BrokerTransformer() {
		SAXParserFactory sparserFactory = SAXParserFactory.newInstance();
		internalBrokerTransformer(sparserFactory);
	}

	private void internalBrokerTransformer(SAXParserFactory sparserFactory) {
	...
	private void addTransformer(String name) {
		...
		StreamSource tranformerXSL = new StreamSource(getClass()
				.getResourceAsStream(XSL_CLASSPATH_BASE + name));
		internalAddTransformer(name, tranformerXSL);

	}

	private void internalAddTransformer(String name, StreamSource tranformerXSL) {
		Transformer tempTransfomer = transformerFactory
				.newTransformer(tranformerXSL);
	...
	private void init(InputSource in) {
		...
		// copy InputSource to CharArrayWriter the so we can re-create the
		// InputSource
		caw = new CharArrayWriter();
		long total = 0;
					...
					public void startElement(String namespace, String name,
						...
						if (!foundFirst) {
							if (name.equals("Invoice")) {
								String schemaLocation = attrs
										.getValue("xsi:schemaLocation");
								if (schemaLocation.endsWith(AIRLINE_INVOICE)) {
									doXSLTTransformation((Transformer) transformers
											.get(AIRLINE_XSL));
								} else if (schemaLocation
										.endsWith(ACTIVITY_INVOICE)) {
									doXSLTTransformation((Transformer) transformers
											.get(ACTIVITY_XSL));
								} else if (schemaLocation
										.endsWith(LODGING_INVOICE)) {
									doXSLTTransformation((Transformer) transformers
											.get(LODGING_XSL));
								}
							}
	...
	private void doXSLTTransformation(Transformer transformer) {
		StreamSource in = new StreamSource(new CharArrayReader(caw
				.toCharArray()));
		ByteArrayOutputStream bos = new ByteArrayOutputStream();
		StreamResult result = new StreamResult(new ByteArrayOutputStream());
		internalDoXSLTTransformation(transformer, in, result);
	}

	private void internalDoXSLTTransformation(Transformer transformer,
			StreamSource in, StreamResult result) {
		if (transformer != null) {
			transformer.transform(in, result);
			String enc = transformer.getOutputProperty(OutputKeys.ENCODING);
			transformedDoc = ((ByteArrayOutputStream) result.getOutputStream())
					.toString(enc);
		} else {
