...
public class LatexRefExtractingParser {
	...
	public void parse(String input) throws IOException {
		...
		LatexLexer lexer = new LatexLexer(new PushbackReader(new StringReader(
				input), 1024));
		...
		internalParse(lexer, expectArg, expectArg2, prevToken, currentCommand,
				argCount);
	}

	private void internalParse(LatexLexer lexer, boolean expectArg,
			boolean expectArg2, Token prevToken,
			TexCommandEntry currentCommand, int argCount) throws IOException {
		for (Token t = lexer.next(); !(t instanceof EOF); t = lexer.next()) {
			if (expectArg) {
				...
				if (t instanceof TArgument) {
					...
					if (argCount == 0) {
						expectArg2 = internalParse(expectArg2, currentCommand,
								t);
					}
					argCount++;
				...
				if (t instanceof TClabel || t instanceof TCbib
						|| t instanceof TCbibstyle || t instanceof TCnew) {
					prevToken = t;
					expectArg = true;
	...
	private boolean internalParse(boolean expectArg2,
