...
public class TexInsertMathSymbolAction extends Action {
	...
	public void run() {
		...
		TexCompletionProposal prop = new TexCompletionProposal(entry, selection
				.getOffset() + 1, 0, editor.getViewer());
		internalRun(selection, doc, prop);
	}

	private void internalRun(ITextSelection selection, IDocument doc,
			TexCompletionProposal prop) {
