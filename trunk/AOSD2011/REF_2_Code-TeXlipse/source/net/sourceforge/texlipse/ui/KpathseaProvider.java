...
public class KpathseaProvider implements ITreeContentProvider, ILabelProvider {
	...
	private class FileType implements ITreeNode {
		...
		public Object[] getChildren() {
			...
			return internalGetChildren(filesearch);
		}

		private Object[] internalGetChildren(KpsewhichRunner filesearch) {
			Kpath[] paths = filesearch.getSearchPaths(parent.getProject(),
					extension);
			KpathNode[] nodes = new KpathNode[paths.length];
			for (int i = 0; i < paths.length; i++)
				nodes[i] = new KpathNode(this, paths[i].path, new ExtFilter(
						extension, paths[i].searchChildren));
			return nodes;
		}
