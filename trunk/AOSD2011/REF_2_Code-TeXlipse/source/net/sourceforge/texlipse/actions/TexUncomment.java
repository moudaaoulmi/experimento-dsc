/*
 * $Id: TexUncomment.java,v 1.1 2005/03/16 12:17:42 kimmok Exp $
 *
...
public class TexUncomment implements IEditorActionDelegate {
	...
	private void uncomment() {
		...
		internalUncomment(strbuf);
	}

	private void internalUncomment(StringBuffer strbuf) {
		// For each line, comment them out
		for (int i = selection.getStartLineIndex(); i <= selection
				.getEndLineIndex(); i++) {
			...
			if (line.startsWith("% ")) {
				strbuf.append(line.replaceFirst("% ", "")
						+ (i < selection.getEndLineIndex() ? selection
								.getEndLineDelim() : ""));
		...
		selection.getDocument().replace(selection.getStartLine().getOffset(),
				selection.getSelLength(), strbuf.toString());
	}
