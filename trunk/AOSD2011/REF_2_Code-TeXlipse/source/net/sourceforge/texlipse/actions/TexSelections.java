...
public class TexSelections {
	...
	public TexSelections(ITextEditor textEditor) {
		...
		// Get the document
		this.document = textEditor.getDocumentProvider().getDocument(
				textEditor.getEditorInput());
		this.editor = textEditor;

		// Get the selection
		this.textSelection = (ITextSelection) this.editor
	...
	private void internalSelect() {
		...
		// selection only
		if (this.selLength > 0) {
			this.startLine = this.document
					.getLineInformation(this.startLineIndex);
			this.endLine = this.document.getLineInformation(this.endLineIndex);

			// Get line delimiter
			this.endLineDelim = this.document
					.getLineDelimiter(this.startLineIndex);

			// Get the selected text
			this.selection = this.document.get(this.textSelection.getOffset(),
					this.textSelection.getLength());
	...
	public void selectCompleteLines() {
		this.selLength = this.endLine.getOffset() + this.endLine.getLength()
				- this.startLine.getOffset();
	}
	...
	public void selectParagraph() {
		if (textSelection.getLength() == 0) {
			internalSelectParagraph();
		} else {
			selectCompleteLines();
		}
	}

	private void internalSelectParagraph() {
	...
	public String getLine(int i) {
		return this.document.get(this.document.getLineInformation(i)
				.getOffset(), this.document.getLineInformation(i).getLength());
	}
	...
	public String getCompleteLines() {
		return document.get(startLine.getOffset(), endLine.getOffset()
				+ this.endLine.getLength() - this.startLine.getOffset());
