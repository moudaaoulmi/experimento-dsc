/*
 * $Id: SpellingMarkerResolution.java,v 1.6 2006/03/04 15:33:37 kimmok Exp $
 *
...
public class SpellingMarkerResolution implements IMarkerResolution2 {
	...
	public void run(IMarker marker) {
		...
		if (charBegin > 0
				&& str.length() < charEnd
				&& (!Character.isWhitespace(str.charAt(charBegin - 1)) || !Character
						.isWhitespace(str.charAt(charEnd)))) {
			charBegin++;
			charEnd++;
			while (str.length() < charEnd
		...
		internalRun(charBegin, charEnd);

		// delete marker
		internalRun(marker);
	}

	private void internalRun(IMarker marker) {
		marker.delete();
	}

	private void internalRun(int charBegin, int charEnd) {
	...
	public String getDescription() {
		return TexlipsePlugin
				.getResourceString("spellCheckerReplaceWithCorrect");
	}
