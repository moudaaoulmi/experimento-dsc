...
public class TexOutlinePage extends ContentOutlinePage {
				...
				.addPropertyChangeListener(new IPropertyChangeListener() {
					...
					public void propertyChange(PropertyChangeEvent event) {
						...
						if (TexlipseProperties.OUTLINE_PART.equals(property)
								|| TexlipseProperties.OUTLINE_CHAPTER
										.equals(property)
								|| TexlipseProperties.OUTLINE_SECTION
										.equals(property)
								|| TexlipseProperties.OUTLINE_SUBSECTION
										.equals(property)
								|| TexlipseProperties.OUTLINE_SUBSUBSECTION
										.equals(property)
								|| TexlipseProperties.OUTLINE_PARAGRAPH
										.equals(property)
								|| TexlipseProperties.OUTLINE_ENVS
										.equals(property)) {
	...
	public void selectionChanged(SelectionChangedEvent event) {
		...
		if (selection.isEmpty()) {
			...
			if (position != null) {
				intenalSelectionChanged(position);
			} else {
				editor.resetHighlightRange();
			}
		}
	...
	private void intenalSelectionChanged(Position position) {
		editor.setHighlightRange(position.getOffset(), position.getLength(),
				true);
	...
	public String getSelectedText() {
		...
		String text = null;
		text = internalGetSelectedText(pos, text);
		return text;
	}

	private String internalGetSelectedText(Position pos, String text) {
		text = this.editor.getDocumentProvider().getDocument(
				this.editor.getEditorInput()).get(pos.getOffset(),
				pos.getLength());
		return text;
	}
	...
	public void removeSelectedText() {
		...
		internalRemoveSelectedText(pos);

		this.editor.updateModelNow();
	}

	private void internalRemoveSelectedText(Position pos) {
		this.editor.getDocumentProvider().getDocument(
				this.editor.getEditorInput()).replace(pos.getOffset(),
				pos.getLength(), "");
	}
	...
	private void internalPaste(String text, Position pos) {
		this.editor.getDocumentProvider().getDocument(
				this.editor.getEditorInput()).replace(
				pos.getOffset() + pos.getLength(), 0, text);
	}
	...
	private void createActions() {
		...
		Action collapse = new Action("Collapse one level",
		...
		collapse.setImageDescriptor(TexlipsePlugin
		...
		Action hideSections = new Action("Hide sections", IAction.AS_CHECK_BOX) {
			public void run() {
				boolean oldState = filter
						.isTypeVisible(OutlineNode.TYPE_SECTION);
		...
		hideSections.setImageDescriptor(TexlipsePlugin
				.getImageDescriptor("hide_sec"));
		this.outlineActions.put(ACTION_HIDE_SEC, hideSections);

		Action hideSubSections = new Action("Hide subsections",
				IAction.AS_CHECK_BOX) {
			public void run() {
				boolean oldState = filter
						.isTypeVisible(OutlineNode.TYPE_SUBSECTION);
		...
		hideSubSections.setImageDescriptor(TexlipsePlugin
				.getImageDescriptor("hide_sub"));
		this.outlineActions.put(ACTION_HIDE_SUBSEC, hideSubSections);

		Action hideSubSubSections = new Action("Hide subsubsections",
				IAction.AS_CHECK_BOX) {
			public void run() {
				boolean oldState = filter
						.isTypeVisible(OutlineNode.TYPE_SUBSUBSECTION);
		...
		hideSubSubSections.setImageDescriptor(TexlipsePlugin
				.getImageDescriptor("hide_subsub"));
		this.outlineActions.put(ACTION_HIDE_SUBSUBSEC, hideSubSubSections);

		Action hideParagraphs = new Action("Hide paragraphs",
				IAction.AS_CHECK_BOX) {
			public void run() {
				boolean oldState = filter
						.isTypeVisible(OutlineNode.TYPE_PARAGRAPH);
		...
		hideParagraphs.setImageDescriptor(TexlipsePlugin
				.getImageDescriptor("hide_para"));
		this.outlineActions.put(ACTION_HIDE_PARAGRAPH, hideParagraphs);

		Action hideFloats = new Action("Hide floating environments",
				IAction.AS_CHECK_BOX) {
			public void run() {
				boolean oldState = filter
						.isTypeVisible(OutlineNode.TYPE_ENVIRONMENT);
				filter.toggleType(OutlineNode.TYPE_ENVIRONMENT, !filter
						.isTypeVisible(OutlineNode.TYPE_ENVIRONMENT));
				TreeViewer viewer = getTreeViewer();
				if (oldState == false) {
		...
		hideFloats.setImageDescriptor(TexlipsePlugin
				.getImageDescriptor("hide_env"));
	...
	private void getOutlinePreferences() {
		...
		boolean part = TexlipsePlugin.getDefault().getPreferenceStore()
				.getBoolean(TexlipseProperties.OUTLINE_PART);
		...
		boolean subsubsection = TexlipsePlugin.getDefault()
		...
		String[] environments = TexlipsePlugin
				.getPreferenceArray(TexlipseProperties.OUTLINE_ENVS);
	...
	private void resetToolbarButtons() {
		((Action) outlineActions.get(ACTION_HIDE_SEC)).setChecked(!filter
				.isTypeVisible(OutlineNode.TYPE_SECTION));
		((Action) outlineActions.get(ACTION_HIDE_SUBSEC)).setChecked(!filter
				.isTypeVisible(OutlineNode.TYPE_SUBSECTION));
		((Action) outlineActions.get(ACTION_HIDE_SUBSUBSEC)).setChecked(!filter
				.isTypeVisible(OutlineNode.TYPE_SUBSUBSECTION));
		((Action) outlineActions.get(ACTION_HIDE_PARAGRAPH)).setChecked(!filter
				.isTypeVisible(OutlineNode.TYPE_PARAGRAPH));
		((Action) outlineActions.get(ACTION_HIDE_FLOAT)).setChecked(!filter
				.isTypeVisible(OutlineNode.TYPE_ENVIRONMENT));
	}
