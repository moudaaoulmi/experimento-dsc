...
/**
 * Calculate number of real lines of code in a method. Does not count blank
...
public class LinesOfCode extends Calculator implements Constants {
	...
	public void calculate(AbstractMetricSource source)
		...
		if (source.getLevel() != METHOD)
			throw new InvalidSourceException(
					"LinesOfCode only applicable to methods");
	...
	public String getSource(MethodDeclaration astNode, AbstractMetricSource mm) {
		...
		return internalGetSource(start, length, unit);
	}

	private String internalGetSource(int start, int length,
	...
	private int calculateLines(String source) {
		...
		if (firstCurly != -1) {
			String body = source.substring(firstCurly + 1, source.length() - 1)
					.trim();
