...
public abstract class DNDHelper {
	...
	protected static Object processReceivedData(DataFlavor flavor,
		...
		if (flavor.equals(DataFlavor.stringFlavor)) {
			receivedData = transferable
					.getTransferData(DataFlavor.stringFlavor);
		} else if (flavor.equals(DataFlavor.javaFileListFlavor)) {
			List aList = (List) transferable
			...
			InputStream is = (InputStream) transferable
					.getTransferData(ASCIIFlavor);
			...
			if (n > 0) {
				 ...
				 * havent checked other platforms. This does not happen with
				 * windows socket io. strange?
				 */
			...
			receivedData = transferable
					.getTransferData(DNDFiguresTransferable.DNDFiguresFlavor);
		}
	 ...
	 * make a new drag gesture recognizer? I do know that if you put for
	 * instance ACTION_COPY but your device supports ACTION_COPY_OR_MOVE, then
	 * the receiving target may show the rejected icon, but will still be forced
	 * improperly to accept your MOVE since the system is not properly calling
	 * your MOVE a MOVE because you claimed incorrectly that you were incapable
	 * of MOVE.
	 */
	...
	protected void setDragGestureListener(
	...
	protected DropTarget createDropTarget() {
		...
		if (Component.class.isInstance(view())) {
			dt = new DropTarget((Component) view(), getDropTargetActions(),
	...
	/**
	 * Used to create the gesture recognizer which in effect turns on
	 * draggability.
	 */
	protected DragGestureRecognizer createDragGestureRecognizer(
		...
		if (Component.class.isInstance(view())) {
			...
			aDgr = java.awt.dnd.DragSource.getDefaultDragSource()
					.createDefaultDragGestureRecognizer(c,
							getDragSourceActions(), dgl);
	...
	/**
	 * Used to destroy the gesture listener which ineffect turns off
	 * dragability.
	 */
...
/**
 * These transferable objects are used to package your data when you want to
 * initiate a transfer. They are not used when you only want to receive data.
 * Formating the data is the responsibility of the sender primarily. Untested.
 * Used for dragging ASCII text out of JHotDraw
 */
/*
 * public class ASCIIText implements Transferable { String s = new
 * String("This is ASCII text"); byte[] bytes;
 * 
 * public DataFlavor[] getTransferDataFlavors() { return new DataFlavor[] {
 * ASCIIFlavor }; }
 * 
 * public boolean isDataFlavorSupported(DataFlavor dataFlavor) { return
 * dataFlavor.equals(ASCIIFlavor); }
 * 
 * public Object getTransferData(DataFlavor dataFlavor) throws
 * UnsupportedFlavorException, IOException { if
 * (!isDataFlavorSupported(dataFlavor)) throw new
 * UnsupportedFlavorException(dataFlavor);
 * 
 * bytes = new byte[s.length() + 1]; for (int i = 0; i < s.length(); i++) bytes
 * = s.getBytes(); bytes[s.length()] = 0; return new
