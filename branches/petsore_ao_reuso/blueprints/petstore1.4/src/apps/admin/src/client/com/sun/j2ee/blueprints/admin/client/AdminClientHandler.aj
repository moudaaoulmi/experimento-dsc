...
/*  //First Attempt
	pointcut internalDateFormatHandler() :
		execution(Date BarChartPanel.internalDateFormat(..));
*/
...
/*	//First attempt - not necessary!
 	pointcut internalCloseOutputStreamHandler() : 
		execution(void HttpPostPetStoreProxy.internalCloseOutputStream(..)); 

	pointcut internalCloseInputStreamHandler() : 
		execution(void HttpPostPetStoreProxy.internalCloseInputStream(..)); 
*/	
...
/*	//First Attempt
	...
	Date around(BarChartPanel bcp) : internalDateFormatHandler() && target(bcp) {	 
	  	try {
	  		bcp.bContinue = true;
	  		return proceed(bcp);
            ...
            //Indicates that execution must stop 
            bcp.bContinue = false;
            return null;
        }
    }
*/
	//Create an "around" advice because the exception is catched but not throwed
	void around(BarChartPanel bcp) : updateModelDatesBarChartPanelHandler() && target(bcp) {	 
	  	try {
	  		proceed(bcp);
        } catch (ParseException e) {
            JOptionPane.showMessageDialog(bcp,
	...
	/*
	//Create an "around" advice because the exception is catched but not throwed
	DocumentBuilder around() : internalGetDocumentBuilderHandler() {	 
	  	try {
	  		return proceed();
	    } catch (Exception e) {
	        e.printStackTrace();
...
/*	//First attempt - not necessary!
 	//Create an "around" advice because the exception is catched but not throwed
	void around() : internalCloseOutputStreamHandler() || internalCloseInputStreamHandler() {	 
	  	try {
	  		proceed();
...
*/	
	//Create an "around" advice because the exception is catched but not throwed
	Document around(HttpPostPetStoreProxy hp, 
			String request, 
    		InputStream ist,
			OutputStream ost,
			HttpURLConnection uc) : internalDoHttpPostHandler() && 
									target(hp) && 
	...
	//Create an "around" advice because the exception is catched but not throwed
	String around() : getStringHandler() {	 
	  	try {
	  		return proceed();
	...
	//Create an "around" advice because the exception is catched but not throwed
	void around(PieChartPanel pcp) : updateModelDatesHandler() && target(pcp) {	 
	  	try {
	  		proceed(pcp);
        } catch (ParseException e) {
            JOptionPane.showMessageDialog(pcp,
                PetStoreAdminClient.getString("DateFormatErrorDialog.message"),
                PetStoreAdminClient.getString("DateFormatErrorDialog.title"),
                JOptionPane.ERROR_MESSAGE);            
        }
	...
	void around(final ServerAction sa) : internalSetValueHandler() && target(sa) {	 
	  	try {
	  		...
	  		//Test is needed!!!
	  		System.out.println("Aspect - AdminClientHandler. Pointcut - internalSetValueHandler. Class: " + sa.getClass().getName());
	  		proceed(sa);
