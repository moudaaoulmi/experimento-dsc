...
public class BracketInserter implements VerifyKeyListener, ILinkedModeListener {
	...
	private class ExitPolicy implements IExitPolicy {
		...
		public ExitPolicy(char exitCharacter, char escapeCharacter,
				Stack<BracketLevel> stack, ISourceViewer viewer) {
		...
		public ExitFlags doExit(LinkedModeModel model, VerifyEvent event,
			...
			if (fSize == fStack.size() && !isMasked(offset)) {
				...
				if (event.character == SWT.CR && offset > 0) {
					...
					return internalDoExit(offset, document, null);
				}
			}
			return null;
		}

		private ExitFlags internalDoExit(int offset, IDocument document,
				ExitFlags exitFlags) {
			if (document.getChar(offset - 1) == '{')
				exitFlags = new ExitFlags(ILinkedModeListener.EXIT_ALL, true);
			return exitFlags;
		}
	...
	private static class ExclusivePositionUpdater implements IPositionUpdater {
		...
		public void update(DocumentEvent event) {
			...
			internalUpdate(event, eventOffset, eventOldLength, eventNewLength,
					deltaLength);
		}

		private void internalUpdate(DocumentEvent event, int eventOffset,
				int eventOldLength, int eventNewLength, int deltaLength) {
	...
	private String getQuotes(boolean opening) {
		String replacement;
		IProject project = ((FileEditorInput) editor.getEditorInput())
				.getFile().getProject();
	...
	public void verifyKey(VerifyEvent event) {
		...
		intenalVerifyKey(event, document, offset, length, character);
	}

	private void intenalVerifyKey(VerifyEvent event, IDocument document,
			final int offset, final int length, final char character) {
		if (document instanceof IDocumentExtension3) {
			if (internalVerifyKey(document, offset) == 0)
				;
		}
		char next = ' ';
		char last = ' ';
		// nao pode ser extraido, pois duas variaves sao dependentes
		last = internal(document, offset, last);
		next = internal1(document, offset, next);
		if (last == '\\')
			return;
		if (character == '"') {
			// Replace quotation marks
			if (!TexlipsePlugin.getDefault().getPreferenceStore().getBoolean(
					TexlipseProperties.SMART_QUOTES))
				return;
		...
		if (character == '\b') {
			if (!TexlipsePlugin.getDefault().getPreferenceStore().getBoolean(
					TexlipseProperties.SMART_BACKSPACE)) {
				return;
			}
		...
		if (character == '.') {
			if (!TexlipsePlugin.getDefault().getPreferenceStore().getBoolean(
					TexlipseProperties.SMART_LDOTS)) {
				return;
			}
		...
		if (last == 't') {
			...
			left = internalVerifyKey(document, offset, left);
		}
		...
		level.fUI.setExitPosition(sourceViewer, offset + level.fLength, 0,
				Integer.MAX_VALUE);
	...
	private char internal1(IDocument document, final int offset, char next) {
		return document.getChar(offset);
	}

	private char internal(IDocument document, final int offset, char last) {
		if (offset > 0)
			last = document.getChar(offset - 1);
		return last;
	}

	private boolean internalVerifyKey(IDocument document, final int offset,
			boolean left) {
	...
	private int internalVerifyKey(IDocument document, final int offset)
					...
					new IDocumentExtension.IReplace() {
						...
						public void perform(IDocument d, IDocumentListener owner) {
							if ((level.fFirstPosition.isDeleted || level.fFirstPosition.length == 0)
								...
								intenalleft(level, document);
							}

							if (fBracketLevelStack.size() == 0) {
								document.removePositionUpdater(fUpdater);
								internalLeft(document);
							}
						}
					});
		}
	...
	private void internalLeft(final IDocument document) {
		document.removePositionCategory(CATEGORY);
	}

	private void intenalleft(final BracketLevel level, final IDocument document) {
