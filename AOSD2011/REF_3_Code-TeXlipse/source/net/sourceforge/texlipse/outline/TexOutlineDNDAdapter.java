/*
 * $Id: TexOutlineDNDAdapter.java,v 1.1 2005/03/16 12:22:11 kimmok Exp $
 *
...
public class TexOutlineDNDAdapter extends ViewerDropAdapter implements
	...
	public boolean performDrop(Object data) {
		...
		int targetOffset = target.getPosition().getOffset()
				+ target.getPosition().getLength();
		...
		internalPerformDrop(data, targetOffset);

		return true;
	}

	private void internalPerformDrop(Object data, int targetOffset) {
	...
	public boolean validateDrop(Object target, int operation,
		...
		if (targetNode == null
				|| targetNode.getType() == OutlineNode.TYPE_PREAMBLE) {
			return false;
		}
	...
	public void dragSetData(DragSourceEvent event) {
		...
		String sourceText = "";
		sourceText = internalDragSetData(sourcePosition, sourceText);

		// set the data
		event.data = sourceText;
	}

	private String internalDragSetData(Position sourcePosition,
			String sourceText) {
		sourceText = getDocument().get(sourcePosition.getOffset(),
	...
	public void dragFinished(DragSourceEvent event) {
		...
		if (event.detail == DND.DROP_MOVE) {
			int sourceLength = this.dragSource.getPosition().getLength();
			internalDragFinished(sourceLength);
		}
	...
	private void internalDragFinished(int sourceLength) {
	...
	private IDocument getDocument() {
		return outline.getEditor().getDocumentProvider().getDocument(
				outline.getEditor().getEditorInput());
	}
	...
	private OutlineNode getSelection() {
		...
		return (OutlineNode) ((IStructuredSelection) selection)
				.getFirstElement();
	}
