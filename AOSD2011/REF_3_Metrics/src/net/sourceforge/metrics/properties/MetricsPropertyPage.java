...
public class MetricsPropertyPage extends PropertyPage implements Constants {
	...
	private void internalCreateContents(IProject p) {
	...
	protected void contributeButtons(Composite parent) {
		...
		addPattern.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				MetricDescriptor md = (MetricDescriptor) table.getSelection()[0]
						.getData("md");
				String[] patterns = (String[]) table.getSelection()[0]
						.getData("patterns");
				ExclusionPatternDialog d = new ExclusionPatternDialog(
						getShell(), md, patterns);
				if (d.open() == IDialogConstants.OK_ID) {
					table.getSelection()[0]
							.setData("patterns", d.getPatterns());
	...
	class EnableMetricsTable extends TableTree implements SelectionListener {
		...
		public void persistState() {
			...
			for (int i = 0; i < items.length; i++) {
				...
				internalPersistState(items, i, id, val, mPatterns);
			}
		}

		private void internalPersistState(TableTreeItem[] items, int i,
				String id, String val, String mPatterns) {
			project.setPersistentProperty(new QualifiedName(
					MetricsPlugin.pluginId, id + ".enabled"), val);
			project.setPersistentProperty(new QualifiedName(
					MetricsPlugin.pluginId, id + ".patterns"), mPatterns);
			TableTreeItem[] folders = items[i].getItems();
			for (int j = 0; j < folders.length; j++) {
				IPackageFragmentRoot f = (IPackageFragmentRoot) folders[j]
						.getData("element");
				String handle = f.getHandleIdentifier();
				String fVal = (folders[j].getChecked()) ? "true" : "false";
				String fPatterns = folders[j].getText(1);
				project.setPersistentProperty(
						new QualifiedName(MetricsPlugin.pluginId, id + "_"
								+ handle + ".enabled"), fVal);
				project.setPersistentProperty(
						new QualifiedName(MetricsPlugin.pluginId, id + "_"
								+ handle + ".patterns"), fPatterns);
			}
		}

		private TableTreeItem createNewRow(MetricDescriptor md,
		...
		private String[] getExclusionPatterns(String id) {
			return getExclusionPatterns(new QualifiedName(
					MetricsPlugin.pluginId, id + ".patterns"));
		}
		 ...
		 */
		private String[] getExclusionPatterns(String id,
				IPackageFragmentRoot folder) {
			String handle = folder.getHandleIdentifier();
			return getExclusionPatterns(new QualifiedName(
					MetricsPlugin.pluginId, id + "_" + handle + ".patterns"));
		}
		...
		private boolean isEnabled(String id, IPackageFragmentRoot folder) {
			...
			if (!mEnabled)
				return false;
			String handle = folder.getHandleIdentifier();
			return internalIsEnabled(id, handle);

		}

		private boolean internalIsEnabled(String id, String handle) {
			String val = project.getPersistentProperty(new QualifiedName(
					MetricsPlugin.pluginId, id + "_" + handle + ".enabled"));
			if (val == null)
				return true;
		...
		private boolean isEnabled(String id) {
			String val = project.getPersistentProperty(new QualifiedName(
					MetricsPlugin.pluginId, id + ".enabled"));
			if (val == null)
				return true;
			return val.equals("true");
		}
		...
		private IPackageFragmentRoot[] getPackageFragmentRoots(IProject project) {
			...
			List result = new ArrayList();
			internalGetPackageFragmentRoots(p, result);
			return (IPackageFragmentRoot[]) result
					.toArray(new IPackageFragmentRoot[] {});
		}

		private void internalGetPackageFragmentRoots(IJavaProject p, List result) {
		...
		private void enableItem(TableTreeItem item, boolean enable, boolean gray) {
			...
			enableDependentMetrics((MetricDescriptor) item.getData("md"),
					enable);
		}
		 ...
		 * @param enable
		 */
		private void enableDependentMetrics(MetricDescriptor descriptor,
				boolean enable) {
			MetricsPlugin plugin = MetricsPlugin.getDefault();
					...
					for (int j = 0; j < items.length; j++) {
						...
						if (id.equals(dependents[i])) {
							if (!enable) { // simply disable
								...
								if (allowEnable(items[j]))
									enableItem(items[j], true, false);
							}
						}
					}
				}
		...
		private boolean allowEnable(TableTreeItem item) {
			...
			if (requires != null && requires.length > 0) {
				for (int i = 0; i < requires.length; i++) {
					TableTreeItem reqItem = (TableTreeItem) rowLookup
							.get(requires[i]);
