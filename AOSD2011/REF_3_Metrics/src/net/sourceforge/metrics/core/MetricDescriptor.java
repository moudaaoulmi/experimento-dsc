...
public class MetricDescriptor {
	...
	private static String[] levels = new String[] { "method", "type",
			"compilationUnit", "packageFragment", "packageFragmentRoot",
			"project" };
	...
	/**
	 * create a list of propagators (AvgValue, MaxValue, Sum) applicable to this
	 * metric as specified in the manifest xml.
	 * 
	 * @return List
	 */
	...
	private void setRange(String minStr, String maxStr, String hint) {
		if (minStr != null) {
			internalSetRange(minStr);
		}
		if (maxStr != null) {
			internalSetRangeMax(maxStr);
		}
		this.hint = hint;
		defaultHint = hint;
		initPreferences();
	...
	private void internalSetRangeMax(String maxStr) {
		max = new Double(maxStr);
		defaultMax = max;
	}

	private void internalSetRange(String minStr) {
	 ...
	 * @return boolean
	 */
	private static boolean getBooleanAttribute(IConfigurationElement element,
	...
	public String getHint() {
		return MetricsPlugin.getDefault().getPreferenceStore().getString(
				getPrefName("HINT"));
	}
	...
	public Double getMax() {
		double max = MetricsPlugin.getDefault().getPreferenceStore().getDouble(
				getPrefName("MAX"));
	...
	public Double getMin() {
		double min = MetricsPlugin.getDefault().getPreferenceStore().getDouble(
				getPrefName("MIN"));
	...
	/**
	 * Check if the given value is within the safe range (boundaries inclusive)
	 * of this metric
	 * 
	 * @param value
	...
	private void initPreferences() {
		if (hint != null)
			MetricsPlugin.getDefault().getPreferenceStore().setDefault(
					getPrefName("HINT"), hint);
		if (min != null)
			MetricsPlugin.getDefault().getPreferenceStore().setDefault(
					getPrefName("MIN"), min.doubleValue());
		if (max != null)
			MetricsPlugin.getDefault().getPreferenceStore().setDefault(
					getPrefName("MAX"), max.doubleValue());
	}

	public void copyToPreferences() {
		if (hint != null)
			MetricsPlugin.getDefault().getPreferenceStore().setValue(
					getPrefName("HINT"), hint);
		if (min != null)
			MetricsPlugin.getDefault().getPreferenceStore().setValue(
					getPrefName("MIN"), min.doubleValue());
	...
	public void resetToDefaults() {
		...
		max = defaultMax;
		min = defaultMin;
		MetricsPlugin.getDefault().getPreferenceStore().setToDefault(
				getPrefName("HINT"));
		MetricsPlugin.getDefault().getPreferenceStore().setToDefault(
				getPrefName("MIN"));
		MetricsPlugin.getDefault().getPreferenceStore().setToDefault(
