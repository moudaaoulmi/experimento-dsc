...
public class NullIdeTaskListManager implements TaskListManager {
	...
	List<SourceLineTask> sourceLineTasks = new ArrayList<SourceLineTask>();
    ...
    public void addSourcelineTask(IMessage message) {
		...
		if (null != message.getSourceLocation()) {
			System.out.println("task: " + message.getMessage() + ", file: " + message.getSourceLocation().getSourceFile().getAbsolutePath()
    ...
    public void addProjectTask(String message, IMessage.Kind kind) {
    	...
    	System.out.println("task: " + message + ", kind: " + kind);	
    }
    ...
    public void clearTasks() {
    	sourceLineTasks = new ArrayList<SourceLineTask>();
        hasWarning = false;
    } 
