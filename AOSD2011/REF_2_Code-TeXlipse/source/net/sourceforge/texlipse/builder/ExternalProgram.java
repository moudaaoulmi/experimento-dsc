/*
 * $Id: ExternalProgram.java,v 1.5 2006/04/04 22:14:05 borisvl Exp $
 *
...
public class ExternalProgram {
	...
	protected String readOutput(InputStream is) {
		...
		internalReadOutput(is, store);
		store.flush();
		return store.getBuffer().toString();
	}

	private void internalReadOutput(InputStream is, StringWriter store) {
	 ...
	 * @throws IOException
	 */
	protected String run(boolean wait, String[] queryMessage)
			throws IOException {
		...
		if ((command != null) && (dir != null)) {
			...
			if (index > 0) {
				...
				envProp.setProperty(key, envProp.getProperty(key)
						+ File.pathSeparatorChar + commandPath);
			}

			String[] env = PathUtils.mergeEnvFromPrefs(envProp,
					TexlipseProperties.BUILD_ENV_SETTINGS);
		...
		final OutputScanner scanner = new OutputScanner(process
				.getInputStream(), process.getOutputStream(), queryMessage,
				consoleOutput);

		// scan also the standard error stream
		final OutputScanner errorScanner = new OutputScanner(process
				.getErrorStream(), process.getOutputStream(), queryMessage,
				consoleOutput);
		...
		outputThread.start();
		errorThread.start();
		internalRun(errorThread, outputThread);
	...
	private void internalRun(final Thread errorThread, final Thread outputThread) {
		// Wait until stream read has finished
		errorThread.join();
		outputThread.join();
	}

	private void internalRun(final Thread errorThread) {
		errorThread.join();
	}
}
