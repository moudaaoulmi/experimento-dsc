 ...
 * ActivityPurchaseOrderBean is a CMP Entity Bean representing a purchase order
 * . It has a 1:MANY relationship with ActivityDetailsBean
 **/
...
public abstract class ActivityPurchaseOrderBean implements EntityBean {
	...
	public void ejbPostCreate(ActivityOrder po) throws CreateException {
		...
		// set activities
		if (po != null) {
			ActivityDetailsLocalHome alh = (ActivityDetailsLocalHome) sl
					.getLocalHome(JNDINames.ACTIVITY_DETAILS_EJB);
			ArrayList activities = po.getActivities();
			for (int i = 0; i < activities.size(); i++) {
				ActivityDetails act = (ActivityDetails) activities.get(i);
				ActivityDetailsLocal al = (ActivityDetailsLocal) alh.create(act
						.getActivityId(), act.getStartDate(), act.getEndDate(),
						act.getHeadCount());
				addActivity(al);
			}
		}
