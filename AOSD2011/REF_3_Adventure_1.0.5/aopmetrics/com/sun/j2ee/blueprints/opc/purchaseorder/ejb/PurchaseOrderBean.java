...
/**
 * Implementation class for the PurchaseOrderBean . PurchaseOrderBean is a CMP
 * Entity Bean representing a purchase order . It has a 1:1 relationship with
 * ContactInfoBean , CreditCardBean, LodgingBean, TransportationBean and a
 * 1:many relationship with ActivityBean
 **/
...
public abstract class PurchaseOrderBean implements EntityBean {
	...
	public void ejbPostCreate(PurchaseOrder po) throws CreateException {
		...
		ContactInfoLocalHome cilh = (ContactInfoLocalHome) sl
				.getLocalHome(JNDINames.CINFO_EJB);
		ContactInfoLocal cil = (ContactInfoLocal) cilh.create(po
				.getShippingInfo());
		setShippingInfo(cil);
		...
		// set credit card
		CreditCardLocalHome cclh = (CreditCardLocalHome) sl
				.getLocalHome(JNDINames.CCARD_EJB);
		CreditCardLocal ccl = (CreditCardLocal) cclh.create(po.getCreditCard());
		setCreditCard(ccl);

		// set lodging
		if (po.getLodging() != null) {
			LodgingLocalHome llh = (LodgingLocalHome) sl
					.getLocalHome(JNDINames.LDG_EJB);
		...
		if (po.getDepartureFlightInfo() != null) {
			TransportationLocalHome tlh = (TransportationLocalHome) sl
					.getLocalHome(JNDINames.TRPN_EJB);
			TransportationLocal tl = (TransportationLocal) tlh.create(po
					.getDepartureFlightInfo());
		...
		if (po.getActivities() != null) {
			ActivityLocalHome alh = (ActivityLocalHome) sl
					.getLocalHome(JNDINames.ACTY_EJB);
	...
	public abstract void setDepartureFlightInfo(
			TransportationLocal departureFlightInfo);

	public abstract void setReturnFlightInfo(
			TransportationLocal returnFlightInfo);
	...
	public PurchaseOrder getPO() {
		...
		if (getDepartureFlightInfo() != null)
			purchaseOrder.setDepartureFlightInfo(getDepartureFlightInfo()
					.getDetails());
		if (getReturnFlightInfo() != null)
			purchaseOrder.setReturnFlightInfo(getReturnFlightInfo()
					.getDetails());
		if (getLodging() != null)
