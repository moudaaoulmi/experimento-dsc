...
/**
 * This is the PO handler that gets called by the OPC Work Flow Manager. The
 * handler first calls the PO receiver that persists the PO. Then it makes a
 * call to the Credit Card Verifier and then the CRM component. Finally it calls
 * the order filler component.
 */
public class POHandler {
	...
	public void handle(Message message) throws HandlerException {
		...
		internalHandle(message, po, poID, emailID, sendMail);
	}

	private void internalHandle(Message message, PurchaseOrder po, String poID,
			String emailID, boolean sendMail) throws HandlerException {
		...
		if (po != null) {
			...
			String lodgOrderStatus = po.getLodging() == null ? OrderStatusNames.COMPLETED
					: OrderStatusNames.PENDING;
			String actyOrderStatus = po.getActivities().length == 0 ? OrderStatusNames.COMPLETED
					: OrderStatusNames.PENDING;
			String airlineOrderStatus = (po.getDepartureFlightInfo() == null)
					&& (po.getReturnFlightInfo() == null) ? OrderStatusNames.COMPLETED
					: OrderStatusNames.PENDING;
			processManager.createManager(poID, OrderStatusNames.PENDING,
					actyOrderStatus, airlineOrderStatus, lodgOrderStatus);
			...
			if (ccStatus) {
				...
				if (JMSUtils.sendMessage(JNDINames.ORDER_FILLER_MDB_QUEUE,
						JNDINames.DOC_TYPE, JNDINames.PO_DOCUMENT, (Object) po) == false) {
					processManager.updateStatus(poID,
							OrderStatusNames.ORDER_FILLER_ERROR);
					processManager.updateOrderErrorStatus(poID, true);
				} else {
					processManager.updateStatus(poID,
							OrderStatusNames.SUBMITTED);
				}
			} else {
	...
	private void sendMail(String emailID, String subject, String msg) {
		...
		JMSUtils.sendMessage(JNDINames.CRM_MDB_QUEUE, JNDINames.DOC_TYPE,
				JNDINames.MAIL_DOCUMENT, xmlMail);
	}
}
