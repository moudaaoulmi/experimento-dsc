...
import com.sun.j2ee.blueprints.waf.controller.*; // tracer import
import com.sun.j2ee.blueprints.util.tracer.Debug;

/**
 * This is the web tier controller for the sample application.
 * 
 * This class is responsible for processing web requests that could originate
 * from any number of front controllers
 * 
 * This class is responsible for ensuring that the dependencies of the client
 * accessing the controller prior to the being passed off to the WebController.
 * 
 */
public class RequestProcessor implements java.io.Serializable {
		...
		if (action != null) {
			...
			if (ev != null) {
				...
				HttpSession session = ((HttpServletRequest) request)
						.getSession();
				ComponentManager cm = (ComponentManager) session
	...
	/**
	 * This method load the necessary Action class necessary to process a the
	 * request for the specified URL. Action instances are cached so that they
	 * may be re-used.
	 */
		...
		if (urlMapping != null) {
			if (urlMapping.isAction()) {
				...
				if ((actionClassString != null)
					...
					handler = internalGetAction(handler, actionClassString);
				}
			}
		}
		return handler;
	...
	private Action internalGetAction(Action handler, String actionClassString) {
		handler = (Action) getClass().getClassLoader().loadClass(
