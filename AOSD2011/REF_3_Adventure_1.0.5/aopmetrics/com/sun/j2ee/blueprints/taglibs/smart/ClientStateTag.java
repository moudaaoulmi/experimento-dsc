...
/**
 * This tag caches state in the page and provides a button or image link within
 * a form with the current page parameters and the page request attributes by
 * encoding them as hidden form variables that are serialized using Base 64
 * Encoded Strings.
 * 
 * This tag when used along with a front controller such as the one provided in
 * the WAF can utilize a flow handler to forward a request page to a page with
 * the same attributes it recieved when the original request was made.
 */
public class ClientStateTag extends BodyTagSupport {
	...
	public int doEndTag() throws JspTagException {
		...
		if (parameters != null) {
			...
			while (it.hasNext()) {
				...
				buffer.append(" <input type=\"hidden\" name=\"" + key
						+ "\" value=\"" + value + "\" />");
			}
		}
		...
		buffer.append(" <input type=\"hidden\" name=\"referring_URL\""
				+ "value=\"" + targetURL + "\">");
		String referringScreen = (String) request.getSession().getAttribute(
				WebKeys.PREVIOUS_SCREEN);
		buffer.append(" <input type=\"hidden\" name=\"referring_screen\""
				+ "value=\"" + referringScreen + "\">");
		buffer.append(" <input type=\"hidden\" name=\"cacheId\"" + "value=\""
				+ cacheId + "\">");
		...
		/**
		 * Now serialize the request attributes into the page (only sealizable
		 * objects are going to be processed).
		 */
			...
			while (enumeration.hasMoreElements()) {
				...
				if (!key.startsWith(cacheId)
					...
					if (serializableClass.isAssignableFrom(value.getClass())) {
						internalDoEndTag(buffer, key, value);
					} else {
	...
	private void internalDoEndTag(StringBuffer buffer) {
		JspWriter out = pageContext.getOut();
		out.print(buffer.toString());
	}

	private void internalDoEndTag(StringBuffer buffer, String key, Object value) {
