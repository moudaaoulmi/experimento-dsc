...
//import com.sun.j2ee.blueprints.opc.powebservice.PurchaseOrderIntf;
...
public final class CheckoutHTMLAction extends HTMLActionSupport {
	...
	private void buildPurchaseOrder(HttpServletRequest request)
		...
		internalBuildPurchaseOrder(request, userId, catalog, cart);
		// clear the adventure package
		cart.clear();
	}

	private void internalBuildPurchaseOrder(HttpServletRequest request,
			String userId, CatalogFacade catalog, Cart cart)
			throws HTMLActionException {
		...
		com.sun.j2ee.blueprints.catalog.Lodging lodging = catalog.getLodging(
				cart.getLodgingId(), locale);
