...
public final class CustomerHTMLAction extends HTMLActionSupport {
	...
	public CustomerBean readAccount(HttpSession session, CustomerFacade facade)
		...
		acct = internalReadAccount(facade, userId, acct);

		return new CustomerBean(acct);
	}

	private Account internalReadAccount(CustomerFacade facade, String userId,
			Account acct) throws CustomerException {
	...
	private CustomerBean createAccount(HttpServletRequest request,
		...
		// call customer component
		internalCreateAccount(facade, acct);

		// return back same data as input
		return new CustomerBean(acct);
	}

	private void internalCreateAccount(CustomerFacade facade,
			com.sun.j2ee.blueprints.customer.Account acct)
			throws CustomerException {
