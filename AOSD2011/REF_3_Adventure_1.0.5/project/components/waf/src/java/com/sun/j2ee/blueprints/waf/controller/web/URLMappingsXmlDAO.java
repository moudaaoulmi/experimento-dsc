		...
		for (int loop = 0; loop < list.getLength(); loop++) {
			...
			if (node != null) {
				...
				if (node instanceof Element) {
					...
					exceptionMappings.add(new ErrorMapping(exceptionClassName,
							screen));
				}
		...
		for (int loop = 0; loop < list.getLength(); loop++) {
			...
			if (node != null) {
				...
				if ((eventClassName != null) && !eventClassName.equals("")) {
					eventMappings.put(eventClassName, new EventMapping(
							eventClassName, commandClassName));
				}
			}
		}
		...
		for (int loop = 0; loop < list.getLength(); loop++) {
			...
			if (node != null) {
				...
				if (node instanceof Element) {
					...
					useFlowHandlerString = element
							.getAttribute(USE_FLOW_HANDLER);
					isTransactionalString = element
							.getAttribute(IS_TRANSACTIONAL);
				}
				webActionClass = getSubTagValue(node, WEB_ACTION_CLASS);
				if (webActionClass != null)
					isAction = true;
				// get request handler
				if ((useFlowHandlerString != null)
						&& useFlowHandlerString.toLowerCase().equals("true"))
					useFlowHandler = true;
				// get flow handler
				if ((useFlowHandlerString != null)
						&& useFlowHandlerString.toLowerCase().equals("true"))
					useFlowHandler = true;
				if ((isTransactionalString != null)
						&& isTransactionalString.toLowerCase().equals("true"))
				...
				if (useFlowHandler) {
					...
					if (node instanceof Element) {
						...
						NodeList children = element
								.getElementsByTagName(FLOW_HANDLER);
						...
						if (children.getLength() > 1) {
							System.err
									.println("Non fatal error: There can be only one <"
											+ FLOW_HANDLER
											+ "> definition in a <"
											+ URL_MAPPING + ">");
						}
						...
						if (flowHandlerNode != null) {
							if (flowHandlerNode instanceof Element) {
								...
								flowHandler = flowElement
										.getAttribute(FLOW_HANDLER_CLASS);
								NodeList results = flowElement
										.getElementsByTagName(HANDLER_RESULT);
								if (results.getLength() > 0) {
									resultMappings = new HashMap();
								}
								for (int resultLoop = 0; resultLoop < results
										.getLength(); resultLoop++) {
									Node resultNode = results.item(resultLoop);
									if (resultNode instanceof Element) {
										Element resultElement = (Element) resultNode;
										String key = resultElement
												.getAttribute(RESULT);
										String value = resultElement
												.getAttribute(NEXT_SCREEN);
										if (!resultMappings.containsKey(key)) {
											resultMappings.put(key, value);
										} else {
											System.err
													.println("*** Non Fatal errror: Screen "
															+ url
															+ " <"
															+ FLOW_HANDLER
															+ "> key "
				...
				URLMapping mapping = new URLMapping(url, screen, isAction,
						useFlowHandler, isTransactional, webActionClass,
						flowHandler, resultMappings);
