 ...
 * com.sun.j2ee.blueprints.waf.controller.Command objects are accessed. A
 * Command is mapped to each com.sun.j2ee.blueprints.waf.controller.Event
 * object. The mapping between the objects is maintained in the mappings.xml
 * file and set by the com.sun.j2ee.blueprints.waf.controller.RequestProcessor
 */
public class CommandFactory implements java.io.Serializable {
	...
	public static Command getCommand(Event ev) {
		...
		if (commandName != null) {
			command = internalGetCommand(commandName, command);
		}
		return command;
	}

	private static Command internalGetCommand(String commandName,
			Command command) {
